...
Already installed? Run `witsos upgrade` to update in place.

Follow [@getwitsos](https://x.com/getwitsos) on X for updates.

### Supercharge Claude Code, Cursor, Codex, OpenCode, Hermes Agent, Gemini, Antigravity, and Kiro with Semantic Code Intelligence

**Surgical context · fewer tool calls · faster answers · 100% local**

### [Documentation & Website →](https://colbymchenry.github.io/witsos/)

[![npm version](https://img.shields.io/npm/v/@colbymchenry/witsos.svg)](https://www.npmjs.com/package/@colbymchenry/witsos)
...
**The WitsOS platform is coming** — for every PR, know exactly what to test, what could break, which flows are affected, and whether business logic is compromised.

<a href="https://getwitsos.com"><img alt="Join the waitlist for early beta access" src="https://raw.githubusercontent.com/colbymchenry/witsos/main/assets/waitlist.svg?v=2" height="52"></a>

<sub>Get <b>early beta access</b> to the hosted product · <a href="https://getwitsos.com">getwitsos.com</a></sub>
...
```bash
npm i -g @colbymchenry/witsos
```

<sub>WitsOS bundles its own runtime — nothing to compile, no native build, works the same everywhere. The installer puts `witsos` on your PATH but **doesn't change your current shell** — open a new terminal before the next step so the command resolves.</sub>

<sub>**Upgrade any time** with `witsos upgrade` — it detects how you installed (bundle, npm, or npx) and updates in place. Add `--check` to see if an update is available, or `witsos upgrade <version>` to pin one.</sub>

</details>

### 2. Wire up your agent(s)

In a **new terminal**, run the installer to connect WitsOS to the agents you use:

```bash
witsos install
```

<sub>Detects and auto-configures Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, and Kiro — wiring the WitsOS MCP server into each. **This is the step that connects WitsOS to your agent;** installing the CLI in step 1 does not do it on its own. It only wires up your agent — it does **not** index any code; building each project's graph is the separate `witsos init` in step 3. (Shortcut: `npx @colbymchenry/witsos` downloads and runs this in one go.)</sub>
...
<sub>`witsos init` creates the local `.witsos/` directory and builds the full graph in the same step — one command, done.</sub>
...
Auto-sync is enabled by default. WitsOS watches the project and updates the graph on every file change — while your agent edits code, or you add, modify, or delete files. **The index is never stale, and there is nothing to re-run.**
...
<sub>Reverses the installer — strips WitsOS's MCP server config, instructions, and permissions from each configured agent. Your project indexes (`.witsos/`) are left untouched; remove those per-project with `witsos uninit`. Use `--target` to remove from specific agents, or `--yes` to run non-interactively.</sub>
...
**WitsOS hands the agent the exact code it needs in one call.** It's a pre-built knowledge graph of every symbol, call edge, and dependency in your codebase — so instead of crawling files, the agent asks one question and gets back the relevant source, the call paths between those symbols (including dynamic-dispatch hops grep can't follow), and the blast radius of a change. **Surgical context, not a file-by-file search** — which means fewer tool calls and faster answers on every codebase, large or small.
...
> **A note on cost:** WitsOS's win on *every* codebase is precision and speed — fewer tool calls, faster answers. It cuts token and dollar cost too, but those savings are **scale-dependent**: small and noisy on a modest codebase, and material only once a repo is large and tangled — at the scale of a Google or Microsoft monorepo, multiplied by a whole team's daily agent usage — for them to compound into a real line item. On a 500-file project, adopt WitsOS for the speed; the cost savings show up when the codebase (and the team) gets big.

### Benchmark Results

Tested across **7 real-world open-source codebases** spanning 7 languages, comparing an agent (Claude Code, headless) answering one architecture question **with** and **without** WitsOS, at the **median of 4 runs per arm**. _Re-validated on Opus 4.8 (2026-06-02), on the current build (`witsos_explore` as the primary tool)._

> **The universal win — every repo, every size: 58% fewer tool calls · 22% faster · file reads cut to ~zero.**

The reliable, universal payoff is **surgical context and speed**: WitsOS collapses the agent's grep/find/Read crawl into a few direct queries — returning the exact methods you asked about even when they're buried in a multi-thousand-line file — so it answers with **near-zero file reads** while the no-WitsOS agent spends its budget on discovery. The **Tokens** and **Cost** columns are real too, but — as noted above — they're **scale-dependent**: small and noisy per query, compounding into real money only at large-codebase, high-volume scale.
...
<sub>**File reads** = median files the agent opened **with** vs **without** WitsOS — the surgical-context win in one column. **Tokens** and **Cost** are the same with-vs-without deltas; they're directional (they move run-to-run) and, per query, small in absolute terms — which is why they only become a line item at scale. `witsos_explore` also collapses redundant interchangeable implementations to signatures, so a response is sized to the *answer* rather than the file count.</sub>
...
**Methodology.** Each arm is `claude -p` (Claude Opus 4.8) run headlessly against the repo with `--strict-mcp-config`: **WITH** = WitsOS's MCP server enabled, **WITHOUT** = an empty MCP config. Built-in Read/Grep/Bash stay available to both. Same question per repo, **4 runs per arm, median reported**. Cost = the run's `total_cost_usd`; Tokens = total tokens processed (input incl. cached + output); Time = wall-clock; Tool calls = every tool invocation, including those inside any sub-agents the model spawns. Repos cloned at `--depth 1` and indexed by the same WitsOS build that served them. Re-validated 2026-06-02 on the current build. These numbers are lower than the prior Opus 4.7 validation — not a WitsOS regression but a stronger native baseline: Opus 4.8 greps/reads efficiently on the main thread instead of fanning out into large Explore-subagent sweeps, so the no-WitsOS arm is leaner than it used to be. Per-repo numbers move run-to-run with how hard the without-arm thrashes (the median-of-4 smooths it, but tails remain — e.g. Django's without-arm hit $2.71/14m one batch).
...
**Why WitsOS wins:** with the index available, the agent answers directly — usually one `witsos_explore` returns the relevant source — and stops, usually with zero file reads. Without it, the agent spends most of its budget on discovery (find/ls/grep) before reading the right code. WitsOS only helps when queried *directly*, so its instructions steer agents to answer directly rather than delegate exploration to file-reading sub-agents — otherwise a sub-agent reads files regardless and WitsOS becomes overhead.
...
<details>
<summary><strong>How auto-syncing works — and why you don't need to run <code>witsos sync</code> manually</strong></summary>

When your agent (Claude Code, Cursor, Codex, opencode) launches `witsos serve --mcp`, three layers keep the index in step with your code — and make sure the agent never gets a silent wrong answer in the brief window between an edit and the next sync:

1. **File watcher with debounced auto-sync.** A native FSEvents / inotify / ReadDirectoryChangesW watcher captures every source-file create / modify / delete and triggers a re-index after a debounce window (default `2000ms`, tunable via `WITSOS_WATCH_DEBOUNCE_MS`, clamped to `[100ms, 60s]`). Bursts of edits collapse into a single sync.
...
**Verify any time** with `witsos status` (CLI). If anything is pending, you'll see a `### Pending sync:` section naming the files and their edit age.

The handful of cases where manual `witsos sync` makes sense: the watcher is disabled (sandboxed environments, or `WITSOS_NO_DAEMON=1`), or you're scripting against the index outside an agent session and want a pre-flight sync at the start of your script.

→ Full deep-dive in [Guides → Indexing a Project](https://colbymchenry.github.io/witsos/guides/indexing/#stay-fresh-automatically).
...
WitsOS detects web-framework routing files and emits `route` nodes linked by `references` edges to their handler classes or functions. Querying callers of a view/controller now surfaces the URL pattern that binds it.
...
- Write each chosen agent's MCP server config, plus a small marker-fenced WitsOS section in the agent's instructions file (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`) — that's how subagents and non-MCP agents learn the `witsos explore` command, since the MCP server's own guidance only reaches the main agent. Removed cleanly by `witsos uninstall`.
- Set up auto-allow permissions when Claude Code is one of the targets

The installer **wires up your agents only — it does not index your code.** After it finishes, build each project's graph yourself with `witsos init` (step 3). One global `witsos install` covers every project; you run `witsos init` once per project.
...
Builds the per-project knowledge graph index, which then auto-syncs on every file change. A single global `witsos install` works in every project you open — no need to re-run the installer per project.

That's it — your agent will use WitsOS tools automatically when a `.witsos/` directory exists.
...
<sub>One wildcard auto-approves every WitsOS tool — `witsos_explore` is the only one listed by default, but if you re-enable others via `WITSOS_MCP_TOOLS` they're already permitted, no prompt.</sub>
...
WitsOS's MCP server delivers its usage guidance to your agent **automatically**, in the MCP `initialize` response. In short, it tells the agent to:

- **Answer structural questions directly with WitsOS** — it *is* the pre-built index, so a grep/read loop just repeats work it already did. Treat the returned source as already read.
- **Reach for `witsos_explore` for almost anything** — "how does X work", a flow/"how does X reach Y", or surveying an area. One call returns the relevant symbols' verbatim source grouped by file, the call paths between them (dynamic-dispatch hops included), and a blast-radius summary. Name a file or symbol in the query to read its current line-numbered source.
- **Trust the results — don't re-verify with grep**, and check the staleness banner after edits.
- Works **per project**: query any project that has a `.witsos/` index by passing `projectPath` — so a monorepo where only some services are indexed, or a second repo, works in one session. A path with no index returns clean guidance to use built-in tools; indexing stays your decision.

The exact text is `src/mcp/server-instructions.ts` — the single source of truth for the main agent. Because subagents and non-MCP harnesses never see the MCP guidance, the installer also writes a short marker-fenced section into the agent's instructions file pointing at the `witsos explore` CLI equivalent.
...
│       calls WitsOS tools directly — no Explore sub-agent       │
│                                 │                                 │
└─────────────────────────────────┬─────────────────────────────────┘
                                  │
                                  ▼
...
2. **Storage** — Everything goes into a local SQLite database (`.witsos/witsos.db`) with FTS5 full-text search.
...
witsos index [path]            # Full index (--force to re-index, --quiet for less output)
witsos sync [path]             # Incremental update
witsos status [path]           # Show statistics
...
witsos files [path]            # Show file structure (--format, --filter, --max-depth, --json)
...
```bash
witsos affected src/utils.ts src/api.ts         # Pass files as arguments
git diff --name-only | witsos affected --stdin   # Pipe from git diff
witsos affected src/auth.ts --filter "e2e/*"     # Custom test file pattern
```
...
```bash
#!/usr/bin/env bash
AFFECTED=$(git diff --name-only HEAD | witsos affected --stdin --quiet)
if [ -n "$AFFECTED" ]; then
...
When running as an MCP server, WitsOS exposes a **single tool** — `witsos_explore`. Measured agent behavior showed that one strong tool steers agents better than a menu of narrower ones — fewer mis-picks, and it saves context every session:

| Tool | Purpose |
|------|---------|
| `witsos_explore` | Answer almost any question in one call — "how does X work", a flow ("how does X reach Y"), or surveying an area — returning the relevant symbols' verbatim source grouped by file, plus the call paths between them and a blast-radius summary. Surfaces dynamic-dispatch hops (callbacks, React re-render, interface→impl) grep can't follow. Name a file or symbol in the query to read its current line-numbered source, the same shape the Read tool gives you. |

The other tools (`witsos_node`, `witsos_search`, `witsos_callers`, `witsos_callees`, `witsos_impact`, `witsos_files`, `witsos_status`) stay fully functional but **unlisted by default** — everything they return already arrives inline on `witsos_explore` (its blast-radius section, the relationship map, a symbol's body as its callee list). Re-enable any of them for the MCP surface with the `WITSOS_MCP_TOOLS` environment variable (e.g. `WITSOS_MCP_TOOLS=explore,node,search,callers`), or use their CLI equivalents (`witsos node` / `query` / `callers` / `callees` / `impact` / `files` / `status`).

Even when the server's own root has no `.witsos/` index, the tools stay available: pass `projectPath` to query any indexed project — a sub-service in a monorepo, or a second repo — in the same session. A path that has no index returns clean guidance to use built-in tools instead, so nothing fails loudly, and indexing stays your decision.

---

## Library Usage

WitsOS can be embedded directly. The npm package re-exports its programmatic
API, so both `import` and `require` resolve the `WitsOS` class in your own
process — handy for embedding it in an app (e.g. an Electron main process).

```typescript
import WitsOS from '@colbymchenry/witsos';
// CommonJS works too:
//   const { WitsOS } = require('@colbymchenry/witsos');

const cg = await WitsOS.init('/path/to/project');
// Or: const cg = await WitsOS.open('/path/to/project');
...
- Install from npm (`npm i @colbymchenry/witsos`) so the matching
...
Next to none — WitsOS is **zero-config by default**, with nothing to write or
...
`static/`), list it under `exclude` in `witsos.json` — gitignore-style
...
those files are skipped by default, because the extension isn't one WitsOS
recognizes. Map them with an optional **`witsos.json`** at your project root:
...
indexing — and a project with no `witsos.json` behaves exactly as before.
Re-index (`witsos index`) after adding or changing mappings.

## Telemetry

WitsOS collects **anonymous usage statistics** — which tools and commands get
...
```bash
witsos telemetry off    # or: WITSOS_TELEMETRY=0, or DO_NOT_TRACK=1
```
...
**"WitsOS not initialized"** — Run `witsos init` in your project directory first.

**Indexing is slow** — Check that `node_modules` and other large directories are excluded. Use `--quiet` to reduce output overhead.

**MCP hits `database is locked`** — current builds shouldn't: WitsOS bundles its own Node runtime and uses Node's built-in `node:sqlite` in WAL mode, where concurrent reads never block on a writer. If you still see it:

- **You're on an old (pre-0.9) install.** Reinstall to get the bundled runtime — `curl -fsSL https://raw.githubusercontent.com/colbymchenry/witsos/main/install.sh | sh` (macOS/Linux), `irm https://raw.githubusercontent.com/colbymchenry/witsos/main/install.ps1 | iex` (Windows), or `npm i -g @colbymchenry/witsos@latest`.
- **`witsos status` shows `Journal:` other than `wal`** — WAL couldn't be enabled on this filesystem (common on network shares and WSL2 `/mnt`), so reads can block on writes. Move the project (with its `.witsos/` folder) onto a local disk.

**MCP server not connecting** — Your agent starts the server itself, so you don't launch it by hand. Make sure the project is initialized and indexed (`witsos status`) and that the path in your MCP config is correct. If it still won't connect, re-run `witsos install` to rewrite the config.

**MCP tool calls fail with `Transport closed` while `witsos status`/`sync` are healthy** — almost always WSL2 with the project on a Windows drive (a `/mnt/c` or `/mnt/d` path), where the local socket WitsOS uses to share one background server across sessions is unreliable. WitsOS now falls back to serving the session in-process instead of dropping the connection, but if you still hit it, set `WITSOS_NO_DAEMON=1` in your MCP server's environment to skip the shared server entirely (each session runs in its own process). Moving the project onto the Linux-native filesystem (e.g. under `~/` instead of `/mnt/`) restores the shared server.

**Missing symbols** — The MCP server auto-syncs on save (wait a couple seconds). Run `witsos sync` manually if needed. Check that the file's language is supported and isn't inside a `.gitignore`d or default-excluded directory (e.g. `node_modules`, `dist`).

**Sharing one checkout between Windows and WSL** — Don't point both at the same `.witsos/`: the background-server lock and the SQLite index are tied to the OS that wrote them, and SQLite locking across the WSL2/Windows filesystem boundary is unreliable. Give each side its own index in the same tree by setting `WITSOS_DIR` to a distinct name on one of them — e.g. `WITSOS_DIR=.witsos-win` on Windows, leaving WSL on the default `.witsos`. WitsOS skips any sibling `.witsos-*` directory when indexing and watching, so the two never trip over each other.

## Star History

<a href="https://www.star-history.com/?repos=colbymchenry%2Fwitsos&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=colbymchenry/witsos&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=colbymchenry/witsos&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=colbymchenry/witsos&type=date&legend=top-left" />
 </picture>
</a>
...
[Report Bug](https://github.com/colbymchenry/witsos/issues) · [Request Feature](https://github.com/colbymchenry/witsos/issues)
