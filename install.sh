...
#
# Upgrade:   run `witsos upgrade` (or just re-run the same command).
# Uninstall: curl -fsSL .../install.sh | sh -s -- --uninstall
#
# Environment:
#   WITSOS_VERSION      release tag to install (default: latest)
#   WITSOS_INSTALL_DIR  bundle location   (default: ~/.witsos)
#   WITSOS_BIN_DIR      symlink location  (default: ~/.local/bin)
set -eu

REPO="colbymchenry/witsos"
INSTALL_DIR="${WITSOS_INSTALL_DIR:-$HOME/.witsos}"
BIN_DIR="${WITSOS_BIN_DIR:-$HOME/.local/bin}"

if [ "${1:-}" = "--uninstall" ]; then
  rm -f "$BIN_DIR/witsos"
  rm -rf "$INSTALL_DIR"
  echo "WitsOS uninstalled (removed $INSTALL_DIR and $BIN_DIR/witsos)."
  exit 0
fi
...
case "$os" in
  Darwin) os="darwin" ;;
  Linux)  os="linux" ;;
  *) echo "witsos: unsupported OS '$os'." >&2; exit 1 ;;
esac
case "$arch" in
  arm64|aarch64) arch="arm64" ;;
  x86_64|amd64)  arch="x64" ;;
  *) echo "witsos: unsupported architecture '$arch'." >&2; exit 1 ;;
esac
target="${os}-${arch}"
...
[ -n "$version" ] || { echo "witsos: could not resolve latest version; set WITSOS_VERSION (e.g. WITSOS_VERSION=v0.9.4)." >&2; exit 1; }
# Release tags are vX.Y.Z; accept a bare X.Y.Z in WITSOS_VERSION too.
case "$version" in v*) ;; *) version="v$version" ;; esac

# 3. Download + extract the bundle.
url="https://github.com/$REPO/releases/download/$version/witsos-${target}.tar.gz"
echo "Installing WitsOS $version ($target)..."
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
curl -fsSL "$url" -o "$tmp/cg.tar.gz" || { echo "witsos: download failed: $url" >&2; exit 1; }

dest="$INSTALL_DIR/versions/$version"
rm -rf "$dest"
mkdir -p "$dest"
# Archives contain a top-level witsos-<target>/ dir; strip it.
tar -xzf "$tmp/cg.tar.gz" -C "$dest" --strip-components=1

# 4. Symlink the launcher onto PATH and mark the current version.
mkdir -p "$BIN_DIR"
ln -sf "$dest/bin/witsos" "$BIN_DIR/witsos"
ln -sfn "$dest" "$INSTALL_DIR/current"

echo "Installed to $dest"
echo "Linked     $BIN_DIR/witsos"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
  *)
    echo ""
