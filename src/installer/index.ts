...
import { getWitsOSDir, WitsOSDirName } from '../directory';
...
/**
 * Interactive entry point — preserves the historical UX (`WitsOS
...
export async function runInstallerWithOptions(opts: RunInstallerOptions): Promise<void> {
  ...
  clack.intro(`WitsOS v${getVersion()}`);
  ...
  // Step 2: install the WitsOS npm package on PATH (always offered;
  // matches existing behavior). Skipped when --yes (assume present).
  if (!useDefaults) {
    const shouldInstallGlobally = await clack.confirm({
      message: 'Install the WitsOS CLI on your PATH? (Required so agents can launch the MCP server)',
      initialValue: true,
    });
    ...
    if (shouldInstallGlobally) {
      ...
      s.start('Installing WitsOS CLI...');
      try {
        execSync('npm install -g @colbymchenry/WitsOS', { stdio: 'pipe', windowsHide: true });
        s.stop('Installed WitsOS CLI on PATH');
      } catch {
        s.stop('Could not install (permission denied)');
        clack.log.warn('Try: sudo npm install -g @colbymchenry/WitsOS');
      }
    } else {
  ...
  if (opts.autoAllow !== undefined) {
    ...
    const ans = await clack.confirm({
      message: 'Auto-allow WitsOS commands? (Skips permission prompts in Claude Code)',
      initialValue: true,
    });
  ...
  // exactly once. Skipped when an env var (DO_NOT_TRACK / WitsOS_TELEMETRY)
  ...
  if (!useDefaults && getTelemetry().getStatus().decidedBy === 'default' && !getTelemetry().hasStoredChoice()) {
    ...
    if (clack.isCancel(share)) {
      ...
      clack.log.info('Skipped — manage anytime with `WitsOS telemetry on|off`.');
    } else {
  ...
  // that runs `WitsOS prompt-hook` — it injects WitsOS_explore context on
  ...
  if (targets.some((t) => t.id === 'claude')) {
    if (useDefaults) {
      ...
      const ans = await clack.confirm({
        message:
          'Front-load WitsOS on “how / where / trace” prompts? Auto-injects structural context so answers need fewer steps (adds a moment to those prompts; Claude Code only).',
        initialValue: true,
      });
      if (clack.isCancel(ans)) {
        clack.cancel('Installation cancelled.');
  ...
  // Building the per-project graph is the user's explicit `WitsOS init`
  ...
  clack.note(
    location === 'local'
      ? 'WitsOS init        # build this project’s graph (one time; auto-syncs after)'
      : 'cd <your-project>\nWitsOS init        # build a project’s graph (one time; auto-syncs after)',
  ...
  const finalNote = targets.length > 0
    ? `Done! Restart your agent${targets.length > 1 ? 's' : ''} to use WitsOS.`
    : 'Done!';
 ...
 * at least one thing; `not-configured` means the agent had no WitsOS
 ...
 * block, permissions) — never the `.WitsOS/` index, which `WitsOS
 * uninit` owns.
 */
export async function runUninstaller(opts: RunUninstallerOptions): Promise<void> {
  const clack = await importESM('@clack/prompts');

  clack.intro(`WitsOS v${getVersion()} — uninstall`);
  ...
  if (opts.location) {
    ...
    const sel = await clack.select({
      message: 'Remove WitsOS from all your projects, or just this one?',
      options: [
  ...
  if (location === 'local' && fs.existsSync(getWitsOSDir(process.cwd()))) {
    clack.log.info(`The ${WitsOSDirName()}/ index for this project is still here. Run \`WitsOS uninit\` to delete it.`);
  }
  ...
  if (removed.length > 0) {
    ...
    clack.outro(
      `Removed WitsOS from ${removed.length} agent${removed.length > 1 ? 's' : ''}: ${names}. ` +
      `Restart ${removed.length > 1 ? 'them' : 'it'} to apply.`,
    );
  } else {
    clack.outro(`WitsOS was not configured in any ${location} agent — nothing to remove.`);
  }
}
...
async function resolveTargets(
  ...
  const choice = await clack.multiselect<string>({
    message: 'Which agents should WitsOS configure?',
 ...
 * /mnt drives, or WitsOS_NO_WATCH), the index would silently go stale.
 * Explain that, and offer to keep it fresh automatically via git hooks
 * (commit / pull / checkout) instead of manual `WitsOS sync`.
 *
...
export async function offerWatchFallback(
  ...
  clack.log.info('Until you re-sync, the WitsOS index stays frozen — it will not pick up edits on its own.');

  // No git repo → the commit-hook path doesn't apply; point at manual sync.
  if (!isGitRepo(projectPath)) {
    clack.log.info('Run `WitsOS sync` after changing files to refresh the index.');
    return;
  }
  ...
  if (opts.yes) {
    ...
    const sel = await clack.select({
      message: 'How should WitsOS keep its index fresh?',
      options: [
        { value: 'hook' as const, label: 'Sync on git commit / pull / checkout', hint: 'installs git hooks (recommended)' },
        { value: 'manual' as const, label: 'I\'ll run `WitsOS sync` myself', hint: 'fully manual' },
      ],
      initialValue: 'hook' as const,
    });
    if (clack.isCancel(sel)) {
      clack.log.info('Skipped — run `WitsOS sync` after changes to refresh the index.');
      return;
    }
    choice = sel;
  }
  ...
  if (result.installed.length > 0) {
    ...
    clack.log.info('Run `WitsOS sync` anytime to refresh immediately.');
  } else {
    clack.log.warn(
      `Could not install git hooks${result.skipped ? ` (${result.skipped})` : ''}. ` +
      'Run `WitsOS sync` after changes instead.',
    );
  }
}
