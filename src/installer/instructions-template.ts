 ...
 *    the WitsOS MCP tools only as deferred names and rarely think to
 *    load them: measured on a forced-delegation flow question (excalidraw,
 *    sonnet, high effort), subagents loaded + used WitsOS in ~1 of 9
 ...
 *    run the `WitsOS explore` CLI, which prints the same output as the
 *    MCP tool.
 *
...
export const WitsOS_SECTION_START = '<!-- WitsOS_START -->';
export const WitsOS_SECTION_END = '<!-- WitsOS_END -->';
 ...
 * indexed" claim would send subagents into failing WitsOS calls (the
