 ...
 *     `./.kiro/settings/mcp.json` (local). Standard `mcpServers.WitsOS`
 *     shape, same as Claude / Cursor / Gemini.
 *   - Instructions to `~/.kiro/steering/WitsOS.md` (global) or
 *     `./.kiro/steering/WitsOS.md` (local). Kiro's "steering" system
 *     loads every `*.md` file in the steering dir as agent context, so
 *     a dedicated `WitsOS.md` is the natural surface — we own the
