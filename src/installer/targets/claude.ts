 ...
 * WitsOS entry out of it on install and strip it on uninstall.
...
class ClaudeCodeTarget implements AgentTarget {
  ...
  install(loc: Location, opts: InstallOptions): WriteResult {
    ...
    // versions wrote `WitsOS mark-dirty` / `sync-if-dirty` hooks to
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // 1b. Also strip the WitsOS entry from a legacy ./.claude.json
    ...
    // 3. Instructions — strip the legacy WitsOS block if present.
 ...
 * `PostToolUse(Edit|Write) → WitsOS mark-dirty` and
 * `Stop → WitsOS sync-if-dirty` (local builds used the
 * `npx @colbymchenry/WitsOS …` form, which still contains the
 * `WitsOS <subcommand>` substring). Both subcommands were later
...
/**
 * Remove stale WitsOS auto-sync hooks from Claude `settings.json`.
 *
 * Surgical at the individual-command level: only entries matching
 * `isLegacyWitsOSHookCommand` are dropped, so a sibling hook sharing
 ...
 * WitsOS command, so a settings.json with no legacy hooks is left
...
/**
 * Remove stale WitsOS auto-sync hooks (`mark-dirty` / `sync-if-dirty`) that a
...
export function cleanupLegacyHooks(loc: Location): WriteResult['files'][number] {
  return removeHookCommandsMatching(loc, isLegacyWitsOSHookCommand);
}
 ...
 * a `command` hook that runs `WitsOS prompt-hook`, which injects
 * WitsOS_explore context for structural prompts so the agent reliably uses
...
/**
 * Strip the marker-delimited WitsOS block from CLAUDE.md if a prior
 * install wrote one. WitsOS no longer maintains an instructions file
