...
export class TreeSitterExtractor {
  ...
  // Value-reference edges (default ON; set WitsOS_VALUE_REFS=0 to disable; see flushValueRefs).
  ...
  private readonly valueRefsEnabled = process.env.WitsOS_VALUE_REFS !== '0';
   ...
   * function_ref), deduped per (reader, target). Default on (WitsOS_VALUE_REFS=0 disables) +
   ...
   * `WitsOS query query_apply_record` returned nothing (issue #634). We emit
...
// Document types (universal knowledge indexing — Phase 2+).
