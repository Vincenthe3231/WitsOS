 ...
 *
 * Defines the tools exposed by the WitsOS MCP server.
 */

import type WitsOS from '../index';
import type { QueryPool } from './query-pool';
import { findNearestWitsOSRoot } from '../directory';
// Lazy-load the heavy WitsOS chain off the MCP startup path — see the same
// helper in engine.ts. ToolHandler must load to answer tools/list (static
// schemas), but it must NOT drag in sqlite/query layers before the daemon binds;
// WitsOS is pulled in only when a tool actually opens a project. require() is
// sync + cached (CommonJS build).
const loadWitsOS = (): typeof import('../index').default =>
...
/**
 * An expected, recoverable "WitsOS can't serve this" condition — most
 ...
 * calling WitsOS entirely (observed repeatedly), which is exactly wrong
 ...
 *
 * Defaults ON. Set `WitsOS_EXPLORE_LINENUMS=0` to disable (used by the
 * A/B harness to measure the payload-cost vs. read-savings tradeoff).
 */
function exploreLineNumbersEnabled(): boolean {
  return process.env.WitsOS_EXPLORE_LINENUMS !== '0';
}
 ...
 * tokio / django / vscode / gin. Set `WitsOS_ADAPTIVE_EXPLORE=0` to disable.
 */
function adaptiveExploreEnabled(): boolean {
  return process.env.WitsOS_ADAPTIVE_EXPLORE !== '0' && process.env.WitsOS_ADAPTIVE_EXPLORE !== 'false';
}
 ...
 *
 * `WitsOS_CATCHUP_GATE_TIMEOUT_MS` overrides the default; `0` restores the
...
function resolveCatchUpGateTimeoutMs(): number {
  const raw = process.env.WitsOS_CATCHUP_GATE_TIMEOUT_MS;
...
export function formatStaleBanner(stale: PendingFile[]): string {
  ...
  return (
    ...
    'their WitsOS entries may be stale:\n' +
    lines.join('\n') +
 ...
 * already names the operator remedy (`WitsOS sync` / git hooks).
 */
export function formatDegradedBanner(reason: string | null): string {
  return (
    '⚠️ WitsOS auto-sync is DISABLED — live file watching stopped, so the index is ' +
...
const projectPathProperty: PropertySchema = {
  type: 'string',
  description: 'Absolute path to the project to query (or any directory inside it) — WitsOS uses the nearest .WitsOS/ index at or above that path. Omit to use this session\'s default project. Pass it to query a second codebase, or when the server root has no index of its own (e.g. a monorepo where only sub-projects are indexed, so there is no default project).',
};
 ...
 * repo, or a monorepo root whose `.WitsOS/` indexes live only in sub-projects
 ...
 * `ToolHandler.getTools()` in the no-WitsOS case (the dynamic per-repo budget
...
export class ToolHandler {
  // Cache of opened WitsOS instances for cross-project queries
  private projectCache: Map<string, WitsOS> = new Map();
  ...
  // .WitsOS/ it resolves to), so the up-to-two `git rev-parse` spawns run
  ...
  constructor(private cg: WitsOS | null) {}
  ...
  /**
   * Update the default WitsOS instance (e.g. after lazy initialization)
   */
  setDefaultWitsOS(cg: WitsOS): void {
    this.cg = cg;
  }
    ...
    try {
      ...
      if (outcome === 'timeout') {
        process.stderr.write(
          `[WitsOS MCP] Catch-up reconcile still running after ${timeoutMs}ms; serving this tool call now and finishing the reconcile in the background (#905). ` +
          `Set WitsOS_CATCHUP_GATE_TIMEOUT_MS=0 to always wait for it.\n`
        );
      }
    } finally {
  ...
  /**
   * Whether a default WitsOS instance is available
   */
  hasDefaultWitsOS(): boolean {
    return this.cg !== null;
  }

  /**
   * Optional allowlist of exposed tools, parsed from the WitsOS_MCP_TOOLS
  ...
  /**
   * Get WitsOS instance for a project
   *
   * If projectPath is provided, opens that project's WitsOS (cached).
   * Otherwise returns the default WitsOS instance.
   *
   * Walks up parent directories to find the nearest .WitsOS/ folder,
   * similar to how git finds .git/ directories.
   */
  private getWitsOS(projectPath?: string): WitsOS {
    if (!projectPath) {
      if (!this.cg) {
        const searched = this.defaultProjectHint ?? process.cwd();
        throw new NotIndexedError(
          'No WitsOS project is loaded for this session.\n' +
          `Searched for a .WitsOS/ directory starting from: ${searched}\n` +
          ...
          '(any project that has a .WitsOS/ — including a sub-project of a monorepo)\n' +
          '  • Or add --path to the server\'s MCP config args: ["serve", "--mcp", "--path", "/absolute/path/to/your/project"]\n' +
          'If a project simply has no index, use your built-in tools (Read/Grep/Glob) for THAT ' +
          "project (the user can run 'WitsOS init' there to enable it) — you can still query " +
    ...
    // real project must still be allowed to resolve UP to its .WitsOS/
    ...
    // Always RE-RESOLVE the nearest .WitsOS/ from the input path. The walk
    ...
    // importantly a git worktree that gained its own .WitsOS/ after the
    ...
    const resolvedRoot = findNearestWitsOSRoot(projectPath);

    if (!resolvedRoot) {
      throw new NotIndexedError(
        `The project at ${projectPath} isn't indexed with WitsOS (no .WitsOS/ directory found ` +
        'walking up from it), so WitsOS cannot query it. Use your built-in tools (Read/Grep/Glob) ' +
        "for that codebase instead, and don't call WitsOS for it again this session. " +
        "Indexing is the user's decision — they can run 'WitsOS init' in that project to enable it."
      );
    }
    ...
    const cg = loadWitsOS().openSync(resolvedRoot);
    this.projectCache.set(resolvedRoot, cg);
    return cg;
  }

  /**
   * Heal a long-lived connection whose `.WitsOS/` was removed and recreated
   * at the same path (a worktree recreated, or `rm -rf .WitsOS` + re-init)
   ...
   * tool call.
   */
  private freshen(cg: WitsOS): WitsOS {
    try {
      if (cg.reopenIfReplaced()) {
        process.stderr.write(
          '[WitsOS MCP] The index was replaced on disk (e.g. a git worktree ' +
  ...
  private worktreeMismatchFor(projectPath?: string): WorktreeIndexMismatch | null {
    ...
    // (cheap — getWitsOS re-walks to the nearest .WitsOS/, no git), then
    ...
    try {
      indexRoot = this.getWitsOS(projectPath).getProjectRoot();
    } catch {
  ...
  private withStalenessNotice(result: ToolResult, projectPath?: string): ToolResult {
    ...
    let cg: WitsOS;
    try {
      cg = this.getWitsOS(projectPath);
    } catch {
      return result; // no default project — leave as is
    }

    // Cross-project `projectPath` calls open a cached WitsOS WITHOUT a
    ...
    // calls open a watcher-less WitsOS, so this is false there — correct: we
    ...
    // Defensive: some test fakes inject a partial WitsOS stub without the
    ...
    for (const p of pending) {
      ...
      // exactly the format both the watcher and every WitsOS response
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      // Honor the optional tool allowlist (WitsOS_MCP_TOOLS): a trimmed
      // surface rejects ablated tools defensively even if a client cached them.
      if (!this.isToolAllowed(toolName)) {
        return this.errorResult(`Tool ${toolName} is disabled via WitsOS_MCP_TOOLS`);
      }
      ...
      return this.errorResult(
        ...
        'This is an internal WitsOS error — retry the call once; if it persists, ' +
        'continue without WitsOS for this task.'
      );
    }
  }
   ...
   */
  private buildFlowFromNamedSymbols(cg: WitsOS, query: string): { text: string; pathNodeIds: Set<string>; namedNodeIds: Set<string>; uniqueNamedNodeIds: Set<string>; spineCallSites: Map<string, number> } {
   ...
   */
  private buildDynamicBoundaries(cg: WitsOS, scanList: Node[], named: Map<string, Node>): string {
   ...
   */
  private buildPolymorphicBoundaries(cg: WitsOS, candidates: Array<{ token: string; family: Node[] }>, named: Map<string, Node>): string {
   ...
   */
  private boundaryCandidates(cg: WitsOS, key: string, keyIsType: boolean, named: Map<string, Node>, selfId: string): string {
   ...
   */
  private buildBlastRadiusSection(cg: WitsOS, subgraph: Subgraph): string {
   ...
   * WitsOS's home turf: relevance by STRUCTURE, not words. A file whose
  ...
  private async handleExplore(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    // cluster are unaffected (no competing mass). Set WitsOS_RANK_NO_MULTITERM=1
    // to disable.
    const MULTITERM_OFF = process.env.WitsOS_RANK_NO_MULTITERM === '1';
    ...
    for (const [filePath, group] of sortedFiles) {
      ...
      // Adaptive sizing (WitsOS_ADAPTIVE_EXPLORE, default on): collapse a file
  ...
  private async handleFileView(
    ...
    if (allFiles.length === 0) return this.textResult('No files indexed. Run `WitsOS index` first.');
    ...
    if (!resolved) {
      return this.textResult(
        `No indexed file matches "${fileArg}". WitsOS indexes source files; configs/docs it doesn't parse won't appear — Read those directly.`,
      );
    }
    ...
    // Compact, one-line blast radius (WitsOS's value-add over a plain Read).
    ...
    if (CONFIG_LEAF_LANGUAGES.has(resolved.language)) {
      ...
      out.push('', '> Values may be secrets, so WitsOS indexes keys only. Read the file directly if you need a value.');
      return this.textResult(this.truncateOutput(out.join('\n')));
    }
  ...
  private async renderNodeSection(cg: WitsOS, node: Node, includeCode: boolean): Promise<string> {
   ...
   */
  private formatTrail(cg: WitsOS, node: Node): string {
    const TRAIL_CAP = 12;
  ...
  private async handleFiles(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (allFiles.length === 0) {
      return this.textResult('No files indexed. Run `WitsOS index` first.');
    }
   ...
   */
  private findSymbolMatches(cg: WitsOS, symbol: string): Node[] {
   ...
   */
  private findAllSymbols(cg: WitsOS, symbol: string): { nodes: Node[]; note: string } {
   ...
   */
  private buildContainerOutline(cg: WitsOS, node: Node): string {
