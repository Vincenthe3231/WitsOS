 ...
 * WitsOS MCP Server
 *
 * Model Context Protocol server that exposes WitsOS functionality
 ...
 *   behavior; used when the user opts out (`WitsOS_NO_DAEMON=1`), no
 *   `.WitsOS/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one WitsOS + watcher + SQLite handle. Spawned on demand; never a
...
import { findNearestWitsOSRoot, getWitsOSDir } from '../directory';
 ...
 */
const DAEMON_INTERNAL_ENV = 'WitsOS_DAEMON_INTERNAL';
...
// are cheap. Paired with deferring the WitsOS load (engine.ts) off the bind
...
/** Whether `WitsOS_NO_DAEMON` was set to a truthy value. */
function daemonOptOutSet(): boolean {
  const raw = process.env.WitsOS_NO_DAEMON;
  if (!raw) return false;
 ...
 * `null` when no `.WitsOS/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.WitsOS/`.
 *
...
function resolveDaemonRoot(explicitPath: string | null): string | null {
  ...
  const root = findNearestWitsOSRoot(candidate);
  if (!root) return null;
 ...
 * `.WitsOS/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    logFd = fs.openSync(path.join(getWitsOSDir(root), 'daemon.log'), 'a');
...
/**
 * MCP Server for WitsOS
 *
 * Implements the Model Context Protocol to expose WitsOS
...
export class MCPServer {
   ...
   *
   * Decision order:
   *   1. `WitsOS_NO_DAEMON=1` → direct mode (unchanged pre-#411 behavior).
   *   2. `WitsOS_DAEMON_INTERNAL=1` → we ARE the detached daemon; listen.
   *   3. No `.WitsOS/` reachable → direct mode (the daemon's lockfile and
   *      socket both live under `.WitsOS/`).
  ...
  async start(): Promise<void> {
    ...
    if (daemonOptOutSet()) {
      return this.startDirect('WitsOS_NO_DAEMON set');
    }
    ...
    if (!root) {
      ...
      return this.startDirect('no .WitsOS/ root found');
    }
    ...
    try {
      ...
      process.stderr.write(`[WitsOS MCP] Proxy path failed (${msg}); falling back to direct mode.\n`);
  ...
  private async startDirect(reason: string): Promise<void> {
    if (reason && process.env.WitsOS_MCP_DEBUG) {
      process.stderr.write(`[WitsOS MCP] Direct mode: ${reason}.\n`);
    }
   ...
   * `WitsOS_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
  ...
  private async startDaemonProcess(): Promise<void> {
    ...
    for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
      ...
      if (existing && existing.pid > 0 && isProcessAlive(existing.pid)) {
        process.stderr.write(
          `[WitsOS daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`
        );
        process.exit(0);
      }
    ...
    process.stderr.write('[WitsOS daemon] Could not acquire the daemon lock; exiting.\n');
    process.exit(0);
  }
  ...
  private installPpidWatchdog(): void {
    ...
    const pollMs = parsePpidPollMs(process.env.WitsOS_PPID_POLL_MS);
    if (pollMs <= 0) return;
    this.ppidWatchdog = setInterval(() => {
      ...
      if (reason) {
        process.stderr.write(
          `[WitsOS MCP] Parent process exited (${reason}); shutting down.\n`
        );
        this.stop();
      }
    }, pollMs);
...
export { WitsOSPackageVersion } from './version';
