 ...
 *
 * One detached `WitsOS serve --mcp` daemon process per project root,
 ...
 *     `WitsOS_DAEMON_IDLE_TIMEOUT_MS` (default 300s) so back-to-back agent
 ...
 *   - The lockfile (`.WitsOS/daemon.pid`) competing daemons arbitrate
...
import { WitsOSPackageVersion } from './version';
...
export interface DaemonHello {
  WitsOS: string; // package version (must match the proxy's own version)
 ...
 * lifecycle. The `WitsOS_client` marker is what tells it apart from the
...
export class Daemon {
  ...
  constructor(
    ...
    // (one stdio client) leaves the pool off; `WitsOS_QUERY_POOL_SIZE=0`
  ...
  async start(): Promise<DaemonStartResult> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      server.listen(this.socketPath, () => {
        ...
        // `.WitsOS/`, which is git-ignored but may be on a shared FS.
        if (process.platform !== 'win32') {
    ...
    const lock: DaemonLockInfo = {
      pid: process.pid,
      version: WitsOSPackageVersion,
      socketPath: this.socketPath,
      startedAt: Date.now(),
    };

    // Drop a discovery record so `WitsOS list` / `stop --all` can find us.
    // Best-effort; a missing record only means list's liveness prune covers it.
    registerDaemon({ root: this.projectRoot, ...lock });

    process.stderr.write(
      `[WitsOS daemon] Listening on ${this.socketPath} (pid ${process.pid}, v${WitsOSPackageVersion}). Idle timeout ${this.idleTimeoutMs}ms.\n`
    );
  ...
  async stop(reason: string = 'stop'): Promise<void> {
    ...
    process.stderr.write(`[WitsOS daemon] Shutting down (${reason}; clients=${this.clients.size}).\n`);
  ...
  reapDeadClients(isAlive: (pid: number) => boolean): number {
    ...
    for (const session of [...this.clients]) {
      ...
      process.stderr.write(
        `[WitsOS daemon] Reaping client with dead peer (pid ${peers.pid}); clients=${this.clients.size - 1}.\n`
      );
      try { session.stop(); } catch { /* best-effort */ }
...
export function tryAcquireDaemonLock(projectRoot: string): AcquireResult {
  ...
  // Make sure the .WitsOS/ directory exists — the daemon may be the first
...
function resolveIdleTimeoutMs(): number {
  const raw = process.env.WitsOS_DAEMON_IDLE_TIMEOUT_MS;
...
function resolveMaxIdleMs(): number {
  const raw = process.env.WitsOS_DAEMON_MAX_IDLE_MS;
...
function resolveClientSweepMs(): number {
  const raw = process.env.WitsOS_DAEMON_CLIENT_SWEEP_MS;
 ...
 * client-hello (carries the `WitsOS_client` marker), or null otherwise — in
...
export function parseClientHelloLine(
  ...
  if (o.WitsOS_client !== 1 || typeof o.pid !== 'number') return null;
