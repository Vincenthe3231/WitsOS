 ...
 * giving the agent a high-level playbook for the WitsOS toolset
 ...
 */
export const SERVER_INSTRUCTIONS = `# WitsOS — code intelligence over an indexed knowledge graph

WitsOS is a SQLite knowledge graph of every symbol, edge, and file in
...
for the same answer. A direct WitsOS answer is typically one to a few
...
- If a tool reports a project isn't indexed (no \`.WitsOS/\`), stop calling WitsOS tools for that project for the rest of the session and use your built-in tools there instead. Indexing is the user's decision — mention they can run \`WitsOS init\` if it comes up, but don't run it yourself.
- Index lags file writes by ~1 second.
- Cross-file resolution is best-effort name matching; ambiguous calls may return multiple candidates.
- No live correctness validation — that's still the TypeScript compiler / test suite / linter's job. WitsOS supplements those with structural context they don't have.
`;

/**
 * Instructions variant sent when the server's own root has NO WitsOS index.
 *
 ...
 * indexed, and a server that started before `WitsOS init` never surfaces the
 * tools afterward). Instead of an "inactive" note, this variant tells the agent
 * WitsOS works **per project**: there's no default project to query, so pass
 * a `projectPath` to any project that HAS a `.WitsOS/`. The full single-
...
This server started somewhere with no \`.WitsOS/\` of its own, so there is no
...
- For a project with no \`.WitsOS/\`, use your built-in tools (Read/Grep/Glob)
  ...
  if it comes up they can run \`WitsOS init\` in a project to enable WitsOS
