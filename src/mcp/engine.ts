 ...
 * the project's {@link WitsOS} instance, file watcher, and the
...
// Lazy-load the heavy WitsOS chain (sqlite + query/graph/context layers) OFF
...
export interface MCPEngineOptions {
   ...
   * single call never pays a worker round-trip. `WitsOS_QUERY_POOL_SIZE=0`
...
export class MCPEngine {
  private cg: WitsOS | null = null;
  private toolHandler: ToolHandler;
  // Project root we resolved to. Null until `ensureInitialized` succeeds
  // (or null forever if no .WitsOS/ ever turned up — that's a valid
   ...
   * mode only; honors `WitsOS_QUERY_POOL_SIZE`). Idempotent and best-effort:
  ...
  private maybeStartPool(root: string): void {
    ...
    const size = resolvePoolSize(process.env.WitsOS_QUERY_POOL_SIZE, os.cpus().length);
    if (size <= 0) {
      process.stderr.write('[WitsOS MCP] Query pool disabled (WitsOS_QUERY_POOL_SIZE=0); serving reads in-process.\n');
      return;
    }
    try {
      this.queryPool = new QueryPool({ root, size });
      this.toolHandler.setQueryPool(this.queryPool);
      process.stderr.write(`[WitsOS MCP] Query pool: up to ${size} worker thread(s) for concurrent reads.\n`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[WitsOS MCP] Query pool unavailable (${msg}); serving reads in-process.\n`);
      this.queryPool = null;
    }
  }
  ...
  /** Whether the default project's WitsOS is open. */
  hasDefaultWitsOS(): boolean {
    return this.toolHandler.hasDefaultWitsOS();
  }

  /**
   * Walk up from `searchFrom` to find the nearest `.WitsOS/` and open it.
  ...
  async ensureInitialized(searchFrom: string): Promise<void> {
    ...
    if (this.toolHandler.hasDefaultWitsOS()) return;
    if (this.initPromise) {
  ...
  retryInitializeSync(searchFrom: string): void {
    ...
    const resolvedRoot = findNearestWitsOSRoot(searchFrom);
    ...
    try {
      ...
      this.cg = loadWitsOS().openSync(resolvedRoot);
      this.projectPath = resolvedRoot;
      this.toolHandler.setDefaultWitsOS(this.cg);
      this.startWatching();
      this.catchUpSync();
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestWitsOSRoot(searchFrom);
    if (!resolvedRoot) {
      // No .WitsOS/ above searchFrom. Sessions may still discover one later via roots/list
    ...
    this.projectPath = resolvedRoot;
    try {
      this.cg = await loadWitsOS().open(resolvedRoot);
      this.toolHandler.setDefaultWitsOS(this.cg);
      this.startWatching();
      this.catchUpSync();
      this.maybeStartPool(resolvedRoot);
      ...
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[WitsOS MCP] Failed to open project at ${resolvedRoot}: ${msg}\n`);
    }
  }

  /**
   * Start file watching on the active WitsOS instance. Idempotent — the
  ...
  private startWatching(): void {
    ...
    if (disabledReason) {
      process.stderr.write(
        `[WitsOS MCP] File watcher disabled — ${disabledReason}. ` +
        `The graph will not auto-update; run \`WitsOS sync\` (or install the git sync hooks via \`WitsOS init\`) to refresh.\n`
      );
    ...
    const debounceMs = parseDebounceEnv(process.env.WitsOS_WATCH_DEBOUNCE_MS);
    if (debounceMs !== undefined) {
      process.stderr.write(`[WitsOS MCP] File watcher debounce: ${debounceMs}ms (WitsOS_WATCH_DEBOUNCE_MS)\n`);
    }
    ...
    const started = this.cg.watch({
      ...
      onSyncComplete: (result) => {
        if (result.filesChanged > 0) {
          process.stderr.write(
            `[WitsOS MCP] Auto-synced ${result.filesChanged} file(s) in ${result.durationMs}ms\n`
          );
        }
      },
      onSyncError: (err) => {
        process.stderr.write(`[WitsOS MCP] Auto-sync error: ${err.message}\n`);
      },
      onDegraded: (reason) => {
        ...
        // (`WitsOS sync` / git sync hooks).
        process.stderr.write(`[WitsOS MCP] File watcher degraded — ${reason}\n`);
      },
    });

    this.watcherStarted = true;
    if (started) {
      process.stderr.write('[WitsOS MCP] File watcher active — graph will auto-sync on changes\n');
    } else {
      process.stderr.write(
        '[WitsOS MCP] File watcher unavailable on this platform — run `WitsOS sync` to refresh the graph after changes.\n'
      );
    }
  }
  ...
  private catchUpSync(): void {
    ...
    const p = cg
      ...
      .then((result) => {
        ...
        if (changed > 0) {
          process.stderr.write(`[WitsOS MCP] Caught up ${changed} file(s) changed since last run\n`);
        }
      })
      .catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[WitsOS MCP] Catch-up sync failed: ${msg}\n`);
      });
...
/**
 * Parse and clamp the WitsOS_WATCH_DEBOUNCE_MS env override.
 *
