 ...
 *
 * One shared `WitsOS serve --mcp` daemon per project root means we need a
 ...
 *
 * Both live under `.WitsOS/` so the project-scoped uninstall (`WitsOS
...
export function getDaemonSocketPath(projectRoot: string): string {
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\WitsOS-${projectHash(projectRoot)}`;
  }
  const inProject = path.join(getWitsOSDir(projectRoot), 'daemon.sock');
  ...
  return path.join(os.tmpdir(), `WitsOS-${projectHash(projectRoot)}.sock`);
}

/** Absolute path to the daemon pid lockfile for `projectRoot`. */
export function getDaemonPidPath(projectRoot: string): string {
  return path.join(getWitsOSDir(projectRoot), 'daemon.pid');
}
