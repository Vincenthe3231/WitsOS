...
export interface DetectInput {
  /** `__filename` of the running CLI module — `<…>/dist/bin/WitsOS.js`. */
  filename: string;
 ...
 * dir so an upgrade reuses a custom `WitsOS_INSTALL_DIR`. Returns null when
...
export function detectInstallMethod(input: DetectInput): InstallMethod {
  ...
  // Bundle: <root>/lib/dist/bin/WitsOS.js → <root> is up 3 from bin/.
  ...
  // npx cache: <…>/_npx/<hash>/node_modules/@colbymchenry/WitsOS/…
  ...
  // Source checkout: running <repo>/dist/bin/WitsOS.js with a sibling .git.
...
export async function resolveLatestVersion(repo = REPO, timeoutMs = 12000): Promise<string> {
  ...
  try {
    const res = await httpsGet(
      ...
      { 'User-Agent': 'WitsOS-upgrade', Accept: 'application/vnd.github+json' },
      timeoutMs
    );
  ...
  throw new Error(
    'could not resolve the latest version from GitHub. Check your network, or pin a version: `WitsOS upgrade <version>`.'
  );
}
...
export interface UpgradeOptions {
  /** Pin a specific version (positional arg or WitsOS_VERSION). */
  version?: string;
...
export async function runUpgrade(opts: UpgradeOptions, deps: UpgradeDeps): Promise<number> {
  ...
  deps.log(`${c.bold('WitsOS')}  current ${c.cyan(currentDisplay)}  ${opts.version ? 'target' : 'latest'} ${c.cyan(latest)}`);
  ...
  if (opts.check) {
    if (updateAvailable) {
      ...
      deps.log(c.dim('Run `WitsOS upgrade` to install it.'));
    } else {
  ...
  if (!updateAvailable && !opts.force && !opts.version) {
    ...
    deps.log(c.dim('Use `--force` to reinstall, or `WitsOS upgrade <version>` to change versions.'));
    return 0;
  }
  ...
  switch (method.kind) {
    ...
    case 'source':
      ...
      return 0;
    default:
      deps.error(`Couldn’t determine how WitsOS was installed (${method.reason}).`);
...
async function selfHealPromptHook(deps: UpgradeDeps): Promise<void> {
  ...
  if (res.action === 'created' || res.action === 'updated') {
    deps.log(
      c.dim('Enabled the WitsOS front-load hook for Claude Code (structural prompts). Disable any time: WitsOS_NO_PROMPT_HOOK=1'),
    );
  }
}
...
export function buildWindowsUpgradeScript(bundleRoot: string, version: string, arch: string): string {
  ...
  // mapped; the NEXT `WitsOS` invocation uses the new one. We can't reuse
  ...
  return [
    ...
    `Write-Host "Installed WitsOS ${version} to $dest"`,
  ].join(';');
}
