 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'WitsOS';
 *
...
/**
 * Base error class for all WitsOS errors.
 *
 * All WitsOS-specific errors extend this class, allowing you to catch
 * all WitsOS errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 *   await cg.indexAll();
 * } catch (error) {
 *   if (error instanceof WitsOSError) {
 *     console.log(`WitsOS error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
export class WitsOSError extends Error {
 ...
 */
export class FileError extends WitsOSError {
 ...
 */
export class ParseError extends WitsOSError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends WitsOSError {
 ...
 */
export class SearchError extends WitsOSError {
 ...
 */
export class VectorError extends WitsOSError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends WitsOSError {
...
/**
 * Simple logger for WitsOS operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.WitsOS_DEBUG) {
      console.debug(`[WitsOS] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[WitsOS] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[WitsOS] ${message}`, context ?? '');
  },
};
