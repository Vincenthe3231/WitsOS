 ...
 * Every WitsOS MCP server on the machine picks it up, so a user configures it
...
/** Managed tier ("WitsOS AI") — the metered gateway used when logged in. */
export const MANAGED_DEFAULT_URL = 'https://ai.getWitsOS.com/v1';
...
export interface OffloadConfig {
  /** Managed tier: route through WitsOS AI (metered) with the logged-in org token. */
  managed?: boolean;
...
export interface ResolvedOffload {
  ...
  enabled: boolean;
  /** Managed tier (WitsOS AI, metered) vs BYO endpoint. */
  managed: boolean;
  url?: string;
  model: string;
...
function configDir(): string {
  return path.join(os.homedir(), '.WitsOS');
}
