 ...
 * reliably crashes WitsOS with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when WitsOS detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    sep,
    `[WitsOS] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    'Node.js 25 or newer has a V8 WASM JIT (turboshaft) Zone allocator bug',
    'that crashes with `Fatal process out of memory: Zone` when WitsOS',
    'compiles tree-sitter grammars. WitsOS WILL crash on this Node',
    'version mid-indexing. See https://github.com/colbymchenry/WitsOS/issues/81',
    '',
    ...
    '  WitsOS_ALLOW_UNSAFE_NODE=1 WitsOS ...',
    sep,
  ].join('\n');
}
 ...
 * package.json. Below this, WitsOS relies on language features / native APIs
...
/**
 * Build the bordered banner shown when WitsOS detects a Node.js major below
...
export function buildNodeTooOldBanner(nodeVersion: string): string {
  ...
  return [
    ...
    `[WitsOS] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    `WitsOS requires Node.js ${MIN_NODE_MAJOR} or newer. Older versions lack`,
    'language features and native APIs WitsOS depends on, and are not',
    'tested or supported.',
    '',
    'Fix: install Node.js 22 LTS:',
    '  nvm install 22 && nvm use 22                          # nvm',
