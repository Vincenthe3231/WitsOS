 ...
 * (e.g. the git-hook `WitsOS sync` running while the MCP server writes).
 */
...
export class DatabaseConnection {
   ...
   * — a git worktree removed and re-added, or `.WitsOS/` deleted and
   ...
   * from what `configureConnection` requested. Surfaced in `WitsOS status` so
   ...
   * The trigger is removing and recreating `.WitsOS/` at the same path under
   * a long-lived process (`git worktree remove` + re-add, or `rm -rf
   * .WitsOS` + `WitsOS init`). Returns false when the inode is unchanged,
 ...
 */
export const DATABASE_FILENAME = 'witsos.db';

/**
 * Get the default database path for a project
 */
export function getDatabasePath(projectRoot: string): string {
  return path.join(getWitsOSDir(projectRoot), DATABASE_FILENAME);
}
