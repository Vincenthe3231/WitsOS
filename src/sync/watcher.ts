...
const EXHAUSTION_REASON =
  'OS watch/file limit exhausted; auto-sync disabled. Run `WitsOS sync` ' +
...
const INOTIFY_LIMIT_REASON =
  ...
  'restart, or run `WitsOS sync` (or install git sync hooks) to refresh.';
 ...
 * log once — partial live-watch (with `WitsOS sync` as the backstop) is far
 * better than exhausting the user's inotify budget and breaking watching
 * system-wide (#579). Tunable via WitsOS_MAX_DIR_WATCHES.
 */
const DEFAULT_MAX_DIR_WATCHES = 50_000;

function maxDirWatches(): number {
  const raw = process.env.WitsOS_MAX_DIR_WATCHES;
  if (raw && /^\d+$/.test(raw)) {
    const n = Number(raw);
    if (n > 0) return n;
  }
...
export class LockUnavailableError extends Error {
  constructor(message = 'WitsOS file lock unavailable; another process is writing') {
    super(message);
 ...
 * - Ignores .WitsOS/ and .git/ regardless of .gitignore
...
export class FileWatcher {
  ...
  start(): boolean {
    ...
    // there; callers fall back to manual `WitsOS sync` or git sync hooks.
  ...
  private isAlwaysIgnored(rel: string): boolean {
    // First path segment. Ignore any WitsOS data dir — the active one AND a
    // sibling like `.WitsOS-win` a second environment (Windows/WSL) created
   ...
   * installed keep firing, and `WitsOS sync` covers the unwatched remainder.
    ...
    try {
      ...
      if (err instanceof LockUnavailableError) {
        ...
        if (this.lockRetryCount > MAX_LOCK_RETRIES) {
          this.degrade(
            'WitsOS file lock held by another process past the retry budget; ' +
              'auto-sync disabled. Run `WitsOS sync` once the other writer finishes ' +
