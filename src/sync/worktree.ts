 ...
 *
 * A WitsOS index lives in a `.WitsOS/` directory and is resolved by
 * walking up parent directories to the nearest one (see
 * `findNearestWitsOSRoot`). That walk is unaware of git worktrees: when a
...
export interface WorktreeIndexMismatch {
  ...
  worktreeRoot: string;
  /** The (different) working tree whose `.WitsOS` index is being used. */
  indexRoot: string;
}

/**
 * Detect when `startPath` lives in one git working tree but the resolved
 * WitsOS index (`indexRoot`) belongs to a *different* working tree.
 *
 ...
 *     that merely happens to contain a `.WitsOS/`), which keeps non-git
...
export function worktreeMismatchWarning(m: WorktreeIndexMismatch): string {
  return (
    `This WitsOS index belongs to a different git working tree.\n` +
    ...
    `symbols changed only here are missing. Run "WitsOS init -i" in this worktree ` +
...
export function worktreeMismatchNotice(m: WorktreeIndexMismatch): string {
  return (
    `⚠ WitsOS results below come from a different git worktree (${m.indexRoot}), ` +
    `not where you're working (${m.worktreeRoot}) — they may reflect another branch, ` +
    `and symbols changed only here are missing. Run "WitsOS init -i" here for a ` +
