 ...
 * see watch-policy.ts), the WitsOS index would otherwise go stale until
 * the user runs `WitsOS sync` by hand. As an opt-in alternative, we can
 ...
 *
 * The hooks run `WitsOS sync` in the background so they never block git,
 * and are guarded by `command -v WitsOS` so they no-op cleanly when the
...
const MARKER_BEGIN = '# >>> WitsOS sync hook >>>';
const MARKER_END = '# <<< WitsOS sync hook <<<';
...
function markerBlock(): string {
  return [
    MARKER_BEGIN,
    '# Keeps the WitsOS index fresh while the live file watcher is off',
    '# (e.g. WSL2 /mnt drives). Runs in the background so it never blocks git.',
    '# Managed by WitsOS; remove with `WitsOS uninit` or delete this block.',
    'if command -v WitsOS >/dev/null 2>&1; then',
    '  ( WitsOS sync >/dev/null 2>&1 & ) >/dev/null 2>&1',
    'fi',
    MARKER_END,
  ].join('\n');
}
...
/**
 * Install (or update) the WitsOS sync hooks in a git repository.
...
/**
 * Remove the WitsOS sync hooks. Strips only our marker block; deletes the
...
/** Whether any WitsOS sync hook is currently installed. */
