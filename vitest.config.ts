...
export default defineConfig({
  test: {
     ...
     * mcp-roots) spawn `dist/bin/witsos.js serve --mcp` with `process.execPath`
     * and rely on the child inheriting `process.env`. On a Node >= 25 dev machine
     * the CLI's hard-block (src/bin/witsos.ts) would otherwise exit the child
     ...
     * users (it's still enforced when `witsos` is invoked directly) while
     ...
     */
    env: {
      WITSOS_ALLOW_UNSAFE_NODE: '1',
      /**
       * The suite spawns real CLI/MCP processes; without this they would write
       * telemetry state into the contributor's real ~/.witsos and count test
