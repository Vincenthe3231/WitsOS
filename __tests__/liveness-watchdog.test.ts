...
describe('installMainThreadWatchdog opt-out', () => {
  it('returns null (spawns nothing) when WITSOS_NO_WATCHDOG is set', () => {
    const prev = process.env.WITSOS_NO_WATCHDOG;
    process.env.WITSOS_NO_WATCHDOG = '1';
    try {
      expect(installMainThreadWatchdog()).toBeNull();
    } finally {
      if (prev === undefined) delete process.env.WITSOS_NO_WATCHDOG;
      else process.env.WITSOS_NO_WATCHDOG = prev;
    }
  });
});
...
describe('liveness watchdog (spawned, real watchdog process)', () => {
  ...
  it('SIGKILLs a process whose main thread wedges in a sync loop', async () => {
    const r = await runChild(
      { WITSOS_WATCHDOG_TIMEOUT_MS: '500' },
  ...
  it('does NOT kill a wedged process when WITSOS_NO_WATCHDOG=1', async () => {
    const { code, signal } = await runChild(
      { WITSOS_WATCHDOG_TIMEOUT_MS: '500', WITSOS_NO_WATCHDOG: '1' },
