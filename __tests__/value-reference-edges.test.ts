 ...
 * "change this constant, affect its readers". Default on; WITSOS_VALUE_REFS=0
...
function valueRefReaders(cg: WitsOS, constName: string): string[] {
...
describe('value-reference edges', () => {
  let dir: string;
  let cg: WitsOS | undefined;

  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-valueref-'));
  });
  afterEach(() => {
    cg?.destroy();
    cg = undefined;
  ...
  function index(): WitsOS {
    const g = WitsOS.initSync(dir, { config: { include: ['**/*.ts', '**/*.tsx'], exclude: [] } });
    return g;
  }
  ...
  it('emits nothing when WITSOS_VALUE_REFS=0', async () => {
    const prev = process.env.WITSOS_VALUE_REFS;
    process.env.WITSOS_VALUE_REFS = '0';
    try {
      fs.writeFileSync(
        path.join(dir, 'config.ts'),
      ...
      if (prev === undefined) delete process.env.WITSOS_VALUE_REFS;
      else process.env.WITSOS_VALUE_REFS = prev;
    }
  });
});
