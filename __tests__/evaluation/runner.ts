...
import { WitsOS } from '../../src/index.js';
...
if (!fs.existsSync(path.join(resolvedPath, '.witsos', 'witsos.db'))) {
  console.error(`No .witsos/witsos.db found at ${resolvedPath}`);
  process.exit(1);
}

let witsosSha = 'unknown';
try {
  witsosSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nWitsOS Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${witsosSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = WitsOS.openSync(resolvedPath);
