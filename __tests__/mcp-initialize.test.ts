 ...
 * the MCP server was blocking the initialize response on WitsOS.open() and
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // daemon (logging to .witsos/daemon.log, not the child's stderr); the
    ...
    env: { ...process.env, WITSOS_NO_DAEMON: '1' },
...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-mcp-init-'));
  });
  ...
  it('responds to initialize quickly when no .witsos exists in cwd', async () => {
  ...
  it('sends initialize response BEFORE tryInitializeDefault finishes', async () => {
    // Seed a real .witsos so the server's tryInitializeDefault path runs
    // its full body: WitsOS.open() (which awaits initGrammars()) and then
    ...
    const json = JSON.parse(response.text);
    expect(json.id).toBe(0);
    expect(json.result.serverInfo.name).toBe('witsos');
  }, 20000);
