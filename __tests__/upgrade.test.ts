...
describe('detectInstallMethod', () => {
  ...
  it('detects a unix bundle and derives the install dir from the versions/ layout', () => {
    const root = '/home/u/.witsos/versions/v0.9.9';
    const filename = `${root}/lib/dist/bin/witsos.js`;
    const present = new Set([`${root}/node`, `${root}/bin/witsos`, '/home/u/.witsos']);
    ...
    expect(m).toEqual({
      ...
      os: 'unix',
      bundleRoot: root,
      installDir: '/home/u/.witsos',
    });
  });

  it('detects a windows bundle and derives the install dir from current\\', () => {
    const root = 'C:/Users/u/AppData/Local/witsos/current';
    const filename = `${root}/lib/dist/bin/witsos.js`;
    const present = new Set([`${root}/node.exe`, `${root}/bin/witsos.cmd`]);
    const m = detectInstallMethod({
      filename,
      platform: 'win32',
    ...
    expect(m.installDir?.replace(/\\/g, '/')).toBe('C:/Users/u/AppData/Local/witsos');
  });

  it('detects a global npm install', () => {
    const filename = '/usr/local/lib/node_modules/@colbymchenry/witsos/dist/bin/witsos.js';
    const m = detectInstallMethod({
      filename,
      platform: 'linux',
      cwd: '/home/u/project',
  ...
  it('detects a local (project) npm install as local', () => {
    ...
    const filename = `${cwd}/node_modules/@colbymchenry/witsos/dist/bin/witsos.js`;
  ...
  it('detects an npx run from the _npx cache', () => {
    const filename = '/home/u/.npm/_npx/abc123/node_modules/@colbymchenry/witsos/dist/bin/witsos.js';
  ...
  it('detects a source checkout via sibling package.json + .git', () => {
    const repo = '/home/u/dev/witsos';
    const filename = `${repo}/dist/bin/witsos.js`;
  ...
  it('returns unknown for an unrecognized layout', () => {
    const m = detectInstallMethod({
      filename: '/opt/weird/place/witsos.js',
      platform: 'linux',
      cwd: '/tmp',
      exists: () => false,
    });
...
describe('deriveInstallDir', () => {
  it('unix: returns the dir above versions/', () => {
    expect(deriveInstallDir('/a/b/.witsos/versions/v1.2.3', 'unix', () => true)).toBe('/a/b/.witsos');
  });
  ...
  it('windows: returns the parent of current\\', () => {
    expect(deriveInstallDir('C:/x/witsos/current', 'windows', () => true)?.replace(/\\/g, '/')).toBe('C:/x/witsos');
  });
  it('windows: null when basename is not current', () => {
    expect(deriveInstallDir('C:/x/witsos/v1', 'windows', () => true)).toBeNull();
  });
});
...
describe('version helpers', () => {
  ...
  it('parseLatestTagFromLocation extracts the tag from a releases redirect', () => {
    expect(parseLatestTagFromLocation('https://github.com/colbymchenry/witsos/releases/tag/v0.9.9')).toBe('v0.9.9');
  ...
  it('reindexAdvisory mentions the refresh commands', () => {
    ...
    expect(a).toContain('witsos sync');
    expect(a).toContain('witsos index -f');
  });
  ...
  it('buildWindowsUpgradeScript targets the right asset per arch and renames-not-deletes the exe', () => {
    ...
    expect(arm).toContain('releases/download/v1.2.3/witsos-win32-arm64.zip');
    ...
    expect(x64).toContain('witsos-win32-x64.zip');
  });
});
...
describe('runUpgrade', () => {
  ...
  it('unix bundle: runs the installer via sh with the derived install dir', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.witsos/versions/v0.9.8', installDir: '/h/.witsos' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
    ...
    expect(calls.runs[0].env?.WITSOS_INSTALL_DIR).toBe('/h/.witsos');
    expect(calls.logs.join('\n')).toMatch(/witsos sync/); // re-index advisory printed
  });

  it('unix bundle: falls back to wget, and errors when neither downloader exists', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.witsos/versions/v0.9.8', installDir: null },
      currentVersion: '0.9.8',
      hasCommand: () => false,
    });
    const code = await runUpgrade({}, deps);
  ...
  it('windows bundle: runs a synchronous in-place (rename + extract) powershell upgrade', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'windows', bundleRoot: 'C:/x/witsos/current', installDir: 'C:/x/witsos' },
      currentVersion: '0.9.8',
      platform: 'win32',
    });
    const code = await runUpgrade({}, deps);
    ...
    expect(decoded).toContain('releases/download/v0.9.9/witsos-win32-');
  ...
  it('source: tells the user to git pull, runs nothing', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'source', root: '/dev/witsos' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
...
describe('index extraction-version stamp / isIndexStale', () => {
  ...
  it('stamps the current extraction version on full index and is not stale', async () => {
    ...
    const cg = await WitsOS.init(dir, { index: false });
