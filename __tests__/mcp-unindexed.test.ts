 ...
 *
 * A server whose own root has no .witsos/ still exposes its tools — gating
 ...
 * `witsos init`. So `initialize` returns the per-project instructions
 ...
 * returns SUCCESS-shaped guidance ("pass projectPath / run witsos init"),
 * never `isError: true` — one or two early isError responses teach an agent to
 * abandon witsos for the whole session, and that failure mode is still
 * guarded below.
 */
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // anyway (the daemon socket lives in .witsos/), and this keeps the
    // suite from leaking a detached daemon in the indexed test.
    // WITSOS_WASM_RELAUNCHED skips the --liftoff-only re-exec: without
    ...
    env: { ...process.env, WITSOS_NO_DAEMON: '1', WITSOS_WASM_RELAUNCHED: '1' },
...
describe('No-root-index session policy', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-unindexed-'));
  });
  ...
  it('initialize returns the per-project instructions (not "inactive", not the full playbook)', async () => {
    ...
    expect(instructions).toMatch(/witsos_explore/);
    expect(instructions).toMatch(/witsos init/);
  ...
  it('tools/list exposes the tools even when the server root has no index (#964)', async () => {
    ...
    expect(tools.map((t) => t.name)).toContain('witsos_explore');
  });
  ...
  it('a query by projectPath reaches an INDEXED sub-project of an unindexed root (monorepo) (#964)', async () => {
    ...
    // sub-project's .witsos/ and return real results. Run through the real
    ...
    const cg = await WitsOS.init(svc, { index: true });
    cg.close();
    ...
    const res = await request(child, {
      id: 1,
      method: 'tools/call',
      params: { name: 'witsos_search', arguments: { query: 'validateToken', projectPath: svc } },
    });
  ...
  it('an INDEXED workspace still gets the full playbook and the explore tool', async () => {
    ...
    const cg = await WitsOS.init(tempDir, { index: true });
    cg.close();
...
describe('No-error policy on expected conditions', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-noerror-'));
  });
  ...
  it('cross-project query to an unindexed path is SUCCESS-shaped guidance, not isError', async () => {
    const res = await new ToolHandler(null).execute('witsos_search', {
      query: 'anything',
      projectPath: tempDir,
    });
    ...
    expect(res.content[0]!.text).toMatch(/witsos init/);
  ...
  it('no-default-project (working-directory detection miss) is SUCCESS-shaped guidance', async () => {
    const res = await new ToolHandler(null).execute('witsos_search', { query: 'anything' });

    expect(res.isError).toBeUndefined();
    expect(res.content[0]!.text).toMatch(/No WitsOS project is loaded/);
...
describe('search kind filter', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-kind-'));
    fs.writeFileSync(
      path.join(tempDir, 'types.ts'),
      'export type PaymentMethod = { id: string };\nexport function pay(): void {}\n'
    );
    cg = await WitsOS.init(tempDir, { index: true });
  });
  ...
  it("kind: 'type' (the advertised enum value) finds type aliases", async () => {
    const res = await new ToolHandler(cg).execute('witsos_search', {
      query: 'PaymentMethod',
      kind: 'type',
    });
