...
describe('issue #238 — connection PRAGMAs (#1)', () => {
  ...
  beforeAll(() => {
    ...
    conn = DatabaseConnection.initialize(path.join(dir, 'witsos.db'));
  });
...
describe('issue #238 — WAL lets a reader proceed during a writer', () => {
  ...
  it('a read on a 2nd connection succeeds while a writer holds the lock', () => {
    const dbPath = path.join(dir, 'witsos.db');
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  beforeAll(async () => {
    ...
    cg = await WitsOS.init(dir, { index: true });
  ...
  it('getWitsOS(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = vi.spyOn(WitsOS, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = (handler as any).getWitsOS(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = (handler as any).getWitsOS(path.join(root, 'does', 'not', 'exist'));
  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('witsos_search', { query: 'helper' }),
        handler.execute('witsos_search', { query: 'helper', projectPath: root }),
        handler.execute('witsos_callers', { symbol: 'helper', projectPath: root }),
        handler.execute('witsos_callees', { symbol: 'main' }),
        handler.execute('witsos_files', { projectPath: root }),
        handler.execute('witsos_status', { projectPath: root }),
      ];
