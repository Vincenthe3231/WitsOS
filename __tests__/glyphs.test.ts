...
describe('supportsUnicode', () => {
  ...
  it('returns false on Windows by default (mojibake-prone consoles)', () => {
    withEnv({ WITSOS_ASCII: undefined, WITSOS_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('win32');
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ WITSOS_ASCII: undefined, WITSOS_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects WITSOS_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ WITSOS_UNICODE: '1', WITSOS_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects WITSOS_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ WITSOS_ASCII: '1', WITSOS_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('WITSOS_ASCII takes precedence over WITSOS_UNICODE', () => {
    withEnv({ WITSOS_ASCII: '1', WITSOS_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns ASCII glyphs on Windows', () => {
    withEnv({ WITSOS_ASCII: undefined, WITSOS_UNICODE: undefined }, () => {
      setPlatform('win32');
      const g = getGlyphs();
