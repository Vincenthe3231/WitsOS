/**
 * witsos_files path-filter normalization (#426)
 *
...
describe('witsos_files path normalization', () => {
  let tempDir: string;
  let cg: WitsOS;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('witsos_files', {
