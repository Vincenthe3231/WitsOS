/**
 * `witsos affected` input-path normalization (#825).
 *
...
describe('witsos affected — input path normalization (#825)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-affected-paths-'));
