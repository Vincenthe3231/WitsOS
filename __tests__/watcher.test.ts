...
describe('FileWatcher', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-watcher-'));
  ...
  describe('watch-resource exhaustion (#876)', () => {
    ...
    const mkEmptyDir = () => fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-exhaust-'));
    ...
    it('warns once (NOT degrade) when Linux inotify watches are exhausted (ENOSPC)', () => {
      ...
      try {
        ...
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-inotify-'));
  ...
  describe('filtering', () => {
    it('should ignore files not matching include patterns', async () => {
      ...
      // it before scheduling sync. (.md is now ingested as a document via
      // chunks_fts, so use a genuinely unsupported extension here.)
      __emitWatchEventForTests(testDir, 'src/readme.bin');
    ...
    it('should ignore .witsos directory changes', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const watcher = newWatcher(syncFn, { debounceMs: 200 });
      ...
      // A .witsos event — FileWatcher's `isAlwaysIgnored` filter must drop
      // it before scheduling sync.
      __emitWatchEventForTests(testDir, '.witsos/db.sqlite');
  ...
  describe('pending file tracking (#403)', () => {
    ...
    it('should retain pending files and retry when syncFn throws LockUnavailableError (#449)', async () => {
      // WitsOS.watch() converts the cross-process lock-failure no-op
  ...
  describe('WitsOS integration', () => {
    ...
    it('should watch and unwatch via WitsOS API', async () => {
