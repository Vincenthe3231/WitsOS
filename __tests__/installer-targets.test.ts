...
// A marker-delimited WitsOS block exactly as a previous installer
...
const LEGACY_BLOCK = [
  ...
  '## WitsOS',
  '',
  'Prefer `witsos_search` / `witsos_callers` over grep.',
  '<!-- WITSOS_END -->',
].join('\n');
        ...
        describe(`location=${location}`, () => {
          ...
          it('install preserves a pre-existing sibling MCP server (where applicable)', () => {
            ...
            if (target.id === 'opencode') {
              ...
              expect(after.mcp.witsos).toBeDefined();
            } else {
...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('codex: install writes config.toml AND the AGENTS.md witsos block (#704)', () => {
    ...
    expect(body).toContain('## WitsOS');
    expect(body).toContain('witsos explore');
  ...
  it('codex: install replaces a legacy AGENTS.md witsos block with the current one, keeping user content', () => {
    const codex = getTarget('codex')!;
    ...
    expect(body).not.toContain('Prefer `witsos_search`');
    expect(body).toContain('witsos explore');
  ...
  it('opencode: prefers .jsonc when both .json and .jsonc exist', () => {
    ...
    expect(jsonText).not.toContain('witsos');
  });
  ...
  it('opencode: preserves line and block comments through install + idempotent re-run', () => {
    ...
    expect(afterInstall).toContain('"witsos"');
  ...
  it('opencode: install writes the AGENTS.md witsos block (#704)', () => {
    const opencode = getTarget('opencode')!;
    const result = opencode.install('global', { autoAllow: true });
    const agentsMd = path.join(tmpHome, '.config', 'opencode', 'AGENTS.md');
    expect(fs.existsSync(agentsMd)).toBe(true);
    expect(fs.readFileSync(agentsMd, 'utf-8')).toContain('witsos explore');
    expect(result.files.find((f) => f.path.endsWith('AGENTS.md'))?.action).toBe('created');
  });

  it('opencode: install replaces a legacy AGENTS.md witsos block, preserving user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
  ...
  it('opencode: uninstall strips a leftover witsos block from AGENTS.md, keeping user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
    ...
    expect(body).toContain('# My personal opencode instructions');
    expect(body).toContain('Always respond in pirate.');
    expect(body).not.toContain('WITSOS_START');
  });
  ...
  it('gemini: install writes settings.json (mcpServers.witsos) and the GEMINI.md block (#704)', () => {
    ...
    expect(fs.readFileSync(geminiMd, 'utf-8')).toContain('witsos explore');
  ...
  it('gemini: uninstall strips witsos but leaves pre-existing settings (security.auth) intact', () => {
    const gemini = getTarget('gemini')!;
    const settings = path.join(tmpHome, '.gemini', 'settings.json');
    fs.mkdirSync(path.dirname(settings), { recursive: true });
    fs.writeFileSync(settings, JSON.stringify({
  ...
  it('gemini: uninstall strips a leftover GEMINI.md witsos block, keeping user content', () => {
    const gemini = getTarget('gemini')!;
    const geminiMd = path.join(tmpHome, '.gemini', 'GEMINI.md');
  ...
  it('kiro: install writes settings/mcp.json (mcpServers.witsos) and no steering doc (#529)', () => {
    ...
    const steering = path.join(tmpHome, '.kiro', 'steering', 'witsos.md');
  ...
  it('kiro: install deletes a leftover steering witsos.md (self-heal) (#529)', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'witsos.md');
  ...
  it('kiro: uninstall strips witsos but leaves sibling MCP servers intact', () => {
    const kiro = getTarget('kiro')!;
    const mcp = path.join(tmpHome, '.kiro', 'settings', 'mcp.json');
    fs.mkdirSync(path.dirname(mcp), { recursive: true });
    fs.writeFileSync(mcp, JSON.stringify({
  ...
  it('kiro: uninstall removes a leftover steering witsos.md file outright', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'witsos.md');
    fs.mkdirSync(path.dirname(steering), { recursive: true });
    fs.writeFileSync(steering, `${LEGACY_BLOCK}\n`);
  ...
  it('kiro: uninstall removes our steering doc but leaves a sibling (product.md) untouched', () => {
    ...
    const ours = path.join(tmpHome, '.kiro', 'steering', 'witsos.md');
  ...
  it('kiro: local install writes ./.kiro/settings/mcp.json and no steering doc (#529)', () => {
    ...
    expect(paths.some((p) => p.endsWith('/.kiro/steering/witsos.md'))).toBe(false);
  });
  ...
  it('antigravity: install writes to LEGACY ~/.gemini/antigravity/mcp_config.json when no migration marker', () => {
    ...
    expect(cfg.mcpServers.witsos).toBeDefined();
  ...
  it('antigravity: entry has NO `type` field (Antigravity rejects entries with it)', () => {
    ...
    expect(cfg.mcpServers.witsos.type).toBeUndefined();
    expect(cfg.mcpServers.witsos.command).toBeDefined();
    expect(cfg.mcpServers.witsos.args).toEqual(['serve', '--mcp']);
  });

  it('antigravity: install migrates a legacy witsos entry to the unified path when marker appears', () => {
    const antigravity = getTarget('antigravity')!;
    // Simulate: user installed on the legacy path, then Antigravity
    // migrated their config (dropped the `.migrated` marker + created
    // the unified file). Re-running witsos install should land
    // witsos in the new file AND strip the stale legacy entry.
    const legacyFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(legacyFile), { recursive: true });
    fs.writeFileSync(legacyFile, JSON.stringify({
      mcpServers: { witsos: { command: 'witsos', args: ['serve', '--mcp'] } },
    }, null, 2) + '\n');
    fs.mkdirSync(path.join(tmpHome, '.gemini', 'config'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.gemini', 'config', '.migrated'), '');
    ...
    expect(unified.mcpServers.witsos).toBeDefined();
    // Legacy file's witsos entry got stripped.
  ...
  it('antigravity: uninstall removes only witsos, sibling MCP server survives', () => {
    const antigravity = getTarget('antigravity')!;
    const mcpFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(mcpFile), { recursive: true });
    fs.writeFileSync(mcpFile, JSON.stringify({
  ...
  it('antigravity: uninstall sweeps BOTH legacy and unified paths (handles migration half-state)', () => {
    const antigravity = getTarget('antigravity')!;
    // User had witsos in BOTH files (e.g. legacy install + post-migration
  ...
  it('gemini + antigravity: both installed coexist (separate MCP files, shared GEMINI.md)', () => {
    ...
    expect(cliCfg.mcpServers.witsos).toBeDefined();
    expect(ideCfg.mcpServers.witsos).toBeDefined();
    ...
    expect(cliAfter.mcpServers.witsos).toBeDefined();
  });

  it('hermes: install adds witsos MCP server and cli toolset, preserving existing yaml', () => {
    ...
    expect(body).toContain('    - mcp-witsos');
  ...
  it('hermes: uninstall removes only witsos MCP server and toolset entry', () => {
    const hermes = getTarget('hermes')!;
    const config = path.join(tmpHome, '.hermes', 'config.yaml');
    fs.mkdirSync(path.dirname(config), { recursive: true });
    ...
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('witsos:');
    expect(body).not.toContain('mcp-witsos');
  ...
  // the next sibling key, truncated the cli block, and spliced `- mcp-witsos`
  ...
  it('hermes: install preserves PyYAML-default list-at-same-indent style (issue #456)', () => {
    ...
    // mcp-witsos appended at the same 2-space indent as existing items
    ...
    // start with `cli:` and not contain a stray 4-space `mcp-witsos`
  ...
  it('hermes: uninstall reverses the install on a PyYAML-default config', () => {
    ...
    expect(installed).toContain('- mcp-witsos');
    expect(installed).toContain('witsos:');

    hermes.uninstall('global');
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('mcp-witsos');
    expect(body).not.toContain('command: witsos');
    expect(body).toContain('  cli:\n  - hermes-cli\n  - browser');
    expect(body).toContain('  telegram:\n  - hermes-telegram');
  });

  it('opencode: uninstall removes only mcp.witsos, preserves comments and siblings', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const file = path.join(dir, 'opencode.jsonc');
    ...
    expect(afterUninstall).not.toContain('witsos');
    expect(afterUninstall).toContain('// important comment');
    expect(afterUninstall).toContain('"other"');
  });

  it('codex: user-added key inside [mcp_servers.witsos] survives idempotent re-install', () => {
    const codex = getTarget('codex')!;
  ...
  it('claude: install creates the CLAUDE.md witsos block (#704)', () => {
    const claude = getTarget('claude')!;
    const result = claude.install('local', { autoAllow: false });
  ...
  it('claude: install replaces a legacy CLAUDE.md witsos block, keeping user content', () => {
    const claude = getTarget('claude')!;
    const claudeMd = path.join(tmpCwd, '.claude', 'CLAUDE.md');
  ...
  it('claude: local install migrates a legacy ./.claude.json witsos entry into ./.mcp.json', () => {
    const claude = getTarget('claude')!;
    const legacy = path.join(tmpCwd, '.claude.json');
    fs.writeFileSync(
      legacy,
      JSON.stringify({ mcpServers: { witsos: { type: 'stdio', command: 'witsos', args: ['serve', '--mcp'] } } }, null, 2),
    );

    claude.install('local', { autoAllow: false });

    // witsos now lives in .mcp.json; the legacy file (which held only
    // witsos) is gone.
    const mcp = JSON.parse(fs.readFileSync(path.join(tmpCwd, '.mcp.json'), 'utf-8'));
    expect(mcp.mcpServers.witsos).toBeDefined();
  ...
  it('claude: legacy ./.claude.json migration preserves sibling servers and unrelated keys', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        mcpServers: {
          witsos: { type: 'stdio', command: 'witsos', args: ['serve', '--mcp'] },
          other: { command: 'x' },
        },
        somethingElse: true,
      }, null, 2),
    ...
    // Only witsos is stripped from the legacy file; siblings survive.
  ...
  it('claude: uninstall strips witsos from ./.mcp.json and a legacy ./.claude.json', () => {
    const claude = getTarget('claude')!;
    // A user left with both the working .mcp.json and a stale .claude.json.
    fs.writeFileSync(
      path.join(tmpCwd, '.mcp.json'),
      JSON.stringify({ mcpServers: { witsos: { command: 'witsos' } } }, null, 2),
    );
    fs.writeFileSync(
      path.join(tmpCwd, '.claude.json'),
      JSON.stringify({ mcpServers: { witsos: { command: 'witsos' }, other: { command: 'x' } } }, null, 2),
    );
    ...
    expect(legacy.mcpServers.witsos).toBeUndefined();
    expect(legacy.mcpServers.other).toBeDefined();
  });

  // ---- Legacy auto-sync hook cleanup ----
  // Pre-0.8 installs wrote `witsos mark-dirty` / `sync-if-dirty`
  ...
  function legacyHookSettings(): Record<string, any> {
    return {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'witsos mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'witsos sync-if-dirty' }] },
  ...
  it('claude: install strips stale witsos auto-sync hooks but keeps the user\'s GitKraken hook', () => {
    const claude = getTarget('claude')!;
    ...
    expect(stopCommands).not.toContain('witsos sync-if-dirty');
    ...
    expect(after.permissions?.allow).toContain('mcp__witsos__*');
  });
    ...
    const file = seedSettings('global', {
      hooks: {
        Stop: [
          {
            hooks: [
              { type: 'command', command: 'witsos sync-if-dirty' },
  ...
  it('claude: cleanupLegacyHooks is a byte-for-byte no-op without witsos hooks', () => {
    const original =
  ...
  it('claude: uninstall strips stale hooks written in the npx form (local)', () => {
    ...
    const file = seedSettings('local', {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'npx @colbymchenry/witsos mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'npx @colbymchenry/witsos sync-if-dirty' }] },
        ],
      },
    });
  ...
  // `witsos prompt-hook`. Must write/remove surgically, be idempotent, and
  ...
  it('claude: install with promptHook:true writes the UserPromptSubmit hook (alongside permissions)', () => {
    ...
    expect(promptCommands(s)).toContain('witsos prompt-hook');
    expect(s.permissions?.allow).toContain('mcp__witsos__*');
  });
  ...
  it('claude: install without promptHook does NOT add the hook', () => {
    ...
    claude.install('global', { autoAllow: true });
    const s = JSON.parse(fs.readFileSync(path.join(tmpHome, '.claude', 'settings.json'), 'utf-8'));
    expect(promptCommands(s)).not.toContain('witsos prompt-hook');
  });
  ...
  it('claude: install with promptHook:true is idempotent (no duplicate, byte-identical re-run)', () => {
    ...
    expect(promptCommands(s).filter((c: string) => c === 'witsos prompt-hook')).toHaveLength(1);
  });
  ...
  it('claude: writePromptHookEntry preserves a sibling UserPromptSubmit hook', () => {
    ...
    expect(promptCommands(s)).toEqual(['my-own-hook', 'witsos prompt-hook']);
  });
  ...
  it('claude: uninstall removes the prompt hook but keeps the user\'s sibling', () => {
    const file = seedSettings('global', {
      hooks: {
        UserPromptSubmit: [
          { hooks: [{ type: 'command', command: 'witsos prompt-hook' }] },
  ...
  it('claude: removePromptHookEntry leaves the legacy auto-sync hook untouched', () => {
    const file = seedSettings('global', {
      hooks: {
        UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'witsos prompt-hook' }] }],
        Stop: [{ hooks: [{ type: 'command', command: 'witsos sync-if-dirty' }] }],
      },
    });
    ...
    expect(stopCmds).toContain('witsos sync-if-dirty');
  });
});
...
describe('Installer targets — TOML serializer (Codex backbone)', () => {
  it('builds a [mcp_servers.witsos] block with command + args', () => {
    const block = buildTomlTable('mcp_servers.witsos', {
      command: 'witsos',
      args: ['serve', '--mcp'],
    });
    expect(block).toContain('[mcp_servers.witsos]');
    expect(block).toContain('command = "witsos"');
    expect(block).toContain('args = ["serve", "--mcp"]');
  });

  it('upsert inserts into empty content', () => {
    const block = buildTomlTable('mcp_servers.witsos', { command: 'witsos', args: ['serve'] });
    const { content, action } = upsertTomlTable('', 'mcp_servers.witsos', block);
    expect(action).toBe('inserted');
    expect(content.startsWith('[mcp_servers.witsos]')).toBe(true);
  });

  it('upsert is idempotent — second call returns unchanged', () => {
    const block = buildTomlTable('mcp_servers.witsos', { command: 'witsos', args: ['serve'] });
    const first = upsertTomlTable('', 'mcp_servers.witsos', block);
    const second = upsertTomlTable(first.content, 'mcp_servers.witsos', block);
  ...
  it('upsert replaces an existing block in place, preserving sibling tables', () => {
    ...
    const newBlock = buildTomlTable('mcp_servers.witsos', {
      command: 'witsos',
      args: ['serve', '--mcp'],
    });
    const { content, action } = upsertTomlTable(existing, 'mcp_servers.witsos', newBlock);
    expect(action).toBe('replaced');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).toContain('[zzz]');
    expect(content).toContain('baz = "qux"');
    expect(content).toContain('command = "witsos"');
    expect(content).not.toContain('old-witsos');
  });
  ...
  it('removeTomlTable strips the block and preserves siblings', () => {
    ...
    const { content, action } = removeTomlTable(existing, 'mcp_servers.witsos');
    expect(action).toBe('removed');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).not.toContain('mcp_servers.witsos');
  });
  ...
  it('upsert preserves an array-of-tables sibling [[foo]]', () => {
    ...
    const block = buildTomlTable('mcp_servers.witsos', { command: 'witsos', args: ['serve'] });
    const { content } = upsertTomlTable(existing, 'mcp_servers.witsos', block);
    expect(content.match(/\[\[foo\]\]/g)?.length).toBe(2);
    expect(content).toContain('[mcp_servers.witsos]');
  });
});

describe('Installer — uninstallTargets sweep (witsos uninstall)', () => {
  let tmpHome: string;
  let tmpCwd: string;
  let origCwd: string;
  let homeRestore: { restore: () => void };
...
describe('Installer — Cursor rules file cleanup on uninstall', () => {
  ...
  const rulesFile = () => path.join(process.cwd(), '.cursor', 'rules', 'witsos.mdc');
  ...
  it('uninstall deletes a leftover witsos.mdc entirely (no orphaned frontmatter left behind)', () => {
  ...
  it('install self-heals a leftover witsos.mdc (#529)', () => {
    plantLegacyRulesFile();
    const result = cursor.install('local', { autoAllow: true });
    expect(fs.existsSync(rulesFile())).toBe(false);
    expect(result.files.some((f) => f.path.endsWith('witsos.mdc') && f.action === 'removed')).toBe(true);
  });

  it('uninstall preserves user content added outside the witsos markers (strips only our block)', () => {
    ...
    expect(after).not.toContain('witsos_search');
    expect(after).not.toContain('WITSOS_START');
  });
});
...
describe('Installer targets — opencode XDG config path (#535)', () => {
  ...
  it('greenfield: targets ~/.config/opencode even when the dir does not exist yet (#535)', () => {
    ...
    // ~/.config/opencode itself on first run; installing witsos FIRST
  ...
  it('install self-heals a pre-#535 %APPDATA% entry, preserving siblings and comments', () => {
    // A previous witsos version wrote into %APPDATA%/opencode. The user
    ...
    fs.writeFileSync(path.join(legacyDir(), 'opencode.jsonc'), [
      ...
      '  "$schema": "https://opencode.ai/config.json",',
      '  "mcp": {',
      '    "witsos": { "type": "local", "command": ["witsos", "serve", "--mcp"], "enabled": true },',
    ...
    expect(legacyText).not.toContain('witsos');
  ...
  it('uninstall sweeps the legacy %APPDATA% entry too (no prior re-install needed)', () => {
    // A user on the broken version goes straight to `witsos uninstall`:
    ...
    expect(fs.readFileSync(path.join(legacyDir(), 'opencode.json'), 'utf-8')).not.toContain('witsos');
