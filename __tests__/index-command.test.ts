/**
 * Regression coverage for issue #874: `witsos index` produced 0 nodes / 0
 * edges while `witsos init` worked, and appeared to wipe the graph.
 *
...
function runWitsOS(args: string[], cwd: string): string {
...
describe('witsos index — full re-index keeps the graph populated (#874)', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-index-cmd-'));
