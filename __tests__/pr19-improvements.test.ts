...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use WitsOS integration)
// =============================================================================
...
describe('Graph Traversal Both Direction', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should traverse both directions from a node', async () => {
    const WitsOS = (await import('../src/index')).default;
...
`);
    ...
    const cg = WitsOS.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'witsos.db');
...
`);
      ...
      const cg = WitsOS.initSync(tmpDir, {
        config: { include: ['src/**/*.ts'], exclude: [] },
      });
      await cg.indexAll();
...
`);
      ...
      // note) so witsos_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via WitsOS.uninitialize()', async () => {
    const WitsOS = (await import('../src/index')).default;

    // Initialize
    const cg = WitsOS.initSync(testDir);
    expect(WitsOS.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .witsos directory should be removed
    expect(WitsOS.isInitialized(testDir)).toBe(false);
  });
});
