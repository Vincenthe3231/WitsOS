/**
 * #383 — WitsOS indexes config KEYS but must never surface config VALUES.
 *
 ...
 * passwords, API keys, JDBC URLs with embedded creds). WitsOS must surface
 * the KEY and never the value — not in node metadata (docstring/signature),
 * not via `witsos_explore`'s verbatim source dump, and not via
 * `witsos_node` `includeCode`. An agent that genuinely needs a value can
 * read the file itself (a deliberate pull); WitsOS must never volunteer it.
 */
...
describe('config secret redaction (#383)', () => {
  ...
  it('witsos_explore surfaces the config key but NEVER the secret value', async () => {
    const res = await handler.execute('witsos_explore', {
  ...
  it('witsos_node includeCode returns the key, not the secret value', async () => {
    const res = await handler.execute('witsos_node', {
