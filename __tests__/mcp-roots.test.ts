 ...
 * failed with a misleading "WitsOS not initialized. Run 'witsos init'."
 *
...
describe('MCP project resolution via roots/list (issue #196)', () => {
  let cwdDir: string;     // where the server is launched — has NO .witsos
  let projectDir: string; // the real indexed project the client reports
  let child: ChildProcessWithoutNullStreams | null = null;

  beforeEach(() => {
    cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-mcp-cwd-'));
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-mcp-proj-'));
  });
  ...
  it('resolves the project from the client roots/list when no rootUri is sent', async () => {
    const cg = await WitsOS.init(projectDir);
    cg.close();
    ...
    send(child, { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'witsos_status', arguments: {} } });
    ...
    expect(text).toContain('WitsOS Status');
    expect(text).not.toContain('No WitsOS project is loaded');
  }, 20000);
  ...
  it('returns an actionable error when there is no rootUri and no roots capability', async () => {
    ...
    expect(text).toContain('No WitsOS project is loaded');
