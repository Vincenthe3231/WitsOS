/**
 * Tests for the `witsos version` affordances.
 *
...
describe('witsos version affordances', () => {
  for (const spelling of ['version', '-v', '-version', '--version', '-V']) {
    it(`\`witsos ${spelling}\` prints exactly the package version`, () => {
  ...
  it('`witsos help` prints usage and the command list', () => {
    const out = run(['help']);
    expect(out).toContain('Usage: witsos');
  ...
  it('a trailing `-v` is still the subcommand\'s --verbose, not the version intercept', () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'witsos-version-test-'));
    let combined = '';
    try {
