...
**Where it flows:** straight into `getImpactRadius` → `witsos impact` and the impact trail
in `witsos_explore` / `witsos_node`. No agent-behaviour change required. **The win is
...
| `valueRefsEnabled` | `process.env.WITSOS_VALUE_REFS !== '0'` — default ON, env opts out. |
...
- **Default ON** for TS/JS/tsx + Go + Python + Rust + Ruby + C + Java + C# (`WITSOS_VALUE_REFS=0` disables). Shipped in **PR #895**
...
- **Tests:** `__tests__/value-reference-edges.test.ts` — same-file readers edged; surfaced in
  ...
  edged (tsx); `WITSOS_VALUE_REFS=0` emits nothing.
...
Index the same repo twice (on vs `WITSOS_VALUE_REFS=0`); node count **must be identical**
...
CG="$(pwd)/dist/bin/witsos.js"
export WITSOS_TELEMETRY=0 DO_NOT_TRACK=1 WITSOS_NO_DAEMON=1
...
WITSOS_VALUE_REFS=0 node "$CG" init "$OFF" 2>&1 | grep -E "nodes,|Indexed"
OND="$ON/.witsos/witsos.db"; OFD="$OFF/.witsos/witsos.db"
...
for s in SOME_CONST ANOTHER_CONST; do
  printf "%-20s ON %s OFF %s\n" "$s" \
    "$(node dist/bin/witsos.js impact "$s" --path "$ON"  2>/dev/null | grep -oE '— [0-9]+ affected' | head -1)" \
    "$(node dist/bin/witsos.js impact "$s" --path "$OFF" 2>/dev/null | grep -oE '— [0-9]+ affected' | head -1)"
done
```
...
`witsos_search`/`callers`, *not* `impact`/`explore`, so it often doesn't query the
value-ref edges at all. ON was never worse than OFF. **So: value-refs does NOT reduce agent
reads — the win is blast-radius correctness** (impact API / WitsOS Pro's verdict engine).
...
- **`WitsOS.initSync(dir, opts)` ignores `opts`** — it takes only the path; the default
