# Design + status: adaptive `witsos_explore` sizing (sibling skeletonization)
...
"Refinement" below). Escape hatch: `WITSOS_ADAPTIVE_EXPLORE=0`.
**Motivation:** make `witsos_explore` size its output to the *answer* rather
...
`witsos_explore` returned full source for **every** relevant file up to its
...
the rest of the session, the WITH-WitsOS arm cost *more* than the WITHOUT arm
...
A file is skeletonized iff **all** hold (and `WITSOS_ADAPTIVE_EXPLORE != 0`):
...
Headless `claude -p`, Opus 4.8, **WITH vs WITHOUT** WitsOS (the real benchmark
