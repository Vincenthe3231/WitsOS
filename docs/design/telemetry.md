...
`witsos telemetry` CLI, MCP + installer wiring, `TELEMETRY.md`. Pending: Worker deploy
+ DNS, release.
Scope: public `witsos` engine (CLI + MCP server + installer)

WitsOS is a local-first tool whose whole pitch is "your code never leaves your machine."
...
- How many machines actively use witsos (daily/weekly), and how does that change?
...
- No telemetry from the `witsos-pro` fork (see "witsos-pro rule" below).
...
4. **First-party endpoint.** Clients only ever talk to `telemetry.getwitsos.com`. The URL
...
| `witsos_version` | `0.9.12` | from package.json |
...
- **`usage_rollup`** — the workhorse. One event per `(day, kind, name)` per machine,
  ...
  (e.g. `witsos_explore`, `affected`), `count`, `error_count`, and for MCP:
...
2. `WITSOS_TELEMETRY=0|1` → forced off/on for that process
3. Global config `~/.witsos/telemetry.json` → stored user choice
...
- **Headless paths** (`npx witsos init`, MCP server — no TTY, never prompt): right
  ...
  **stderr** and record `first_run_notice_shown`:
  `witsos collects anonymous usage stats (no code or paths) — "witsos telemetry off" or WITSOS_TELEMETRY=0 disables. Details: TELEMETRY.md`
- **CLI:** `witsos telemetry status|on|off` (status prints the machine ID, current
  state, and what decided it). Deleting `~/.witsos/telemetry.json` resets everything,
  including the machine ID.

`~/.witsos/telemetry.json`:
...
(`~/.witsos/` is new — today nothing global exists. Coexists by filename if a user ever
indexes `$HOME` itself, since per-project data lives in `<project>/.witsos/` with fixed
other filenames.)
...
- **Buffer** — counters persist (debounced, async) to `~/.witsos/telemetry-queue.jsonl`.
...
- **Flush** — many CLI actions end via `process.exit()`, where `beforeExit` never fires
  ...
  `https://telemetry.getwitsos.com/v1/events` with `AbortSignal.timeout(1500)`,
  ...
  sender's claim merges back after an hour). `WITSOS_TELEMETRY_DEBUG=1` echoes
...
`telemetry.getwitsos.com` → small Worker living at `telemetry-worker/` in this repo —
...
## witsos-pro rule (do not lose this in upstream merges)

The private `witsos-pro` fork ships inside customer containers whose guarantee is
...
container sets `WITSOS_TELEMETRY=0` as belt-and-braces. This rule lives in the fork's
...
2. Worker + DNS live first (so the first shipping client never 404s), PostHog dashboards:
   ...
   languages indexed.
3. Client module + config + `witsos telemetry` subcommand + MCP `clientInfo` plumbing.
