...
# Env: CORPUS  corpus dir (default: /tmp/witsos-corpus)
set -uo pipefail
...
REPO_ROOT="$(cd "$HARNESS/../.." && pwd)"     # witsos repo root
CORPUS="${CORPUS:-/tmp/witsos-corpus}"
REPO="$CORPUS/$NAME"
PKG="@colbymchenry/witsos"

echo "==================== WitsOS audit ===================="
echo "version=$VERSION  repo=$NAME  mode=$MODE  corpus=$CORPUS"
echo

# 1. Set the witsos version under test (mutates the global install).
...
ACTUAL="$(witsos --version 2>/dev/null || echo '?')"
echo "  witsos on PATH: $(command -v witsos) -> $ACTUAL"
...
echo "→ [3/4] wiping .witsos and re-indexing with $ACTUAL"
rm -rf "$REPO/.witsos"
...
if ( cd "$REPO_ROOT" && ./scripts/local-install.sh >/dev/null 2>&1 ); then
  echo "  global witsos restored to dev build"
else
