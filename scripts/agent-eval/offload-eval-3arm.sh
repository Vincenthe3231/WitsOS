...
#   ARM offload : witsos attached, managed offload ON  (per-run AI usage log)
#   ARM raw     : witsos attached, WITSOS_OFFLOAD_DISABLE=1 (raw source)
#   ARM nocg    : no witsos (empty MCP config) -> Read/Grep baseline
...
[ -d "$TARGET/.witsos" ] || { echo "not indexed: $TARGET (run offload-eval-setup.sh first)"; exit 1; }
...
run() { # arm rep mcp-config usage-log-or-dash
  ...
  # measures the retrieval mode, not whether Sonnet decides to spawn a witsos-blind
...
printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","WITSOS_OFFLOAD_USAGE_LOG=%s","node","%s","serve","--mcp","--path","%s"]}}}' "$USAGE" "$BIN" "$TARGET" > "$CFG_OFF"
printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","WITSOS_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG_RAW"
...
echo "###### Q=$Q"
echo "== ARM offload =="; prewarm "$TARGET" "WITSOS_OFFLOAD_USAGE_LOG=$USAGE"
for r in $(seq "$START" "$END"); do run offload "$r" "$CFG_OFF" "$USAGE"; done
pkill -9 -f "serve --mcp --path $TARGET" 2>/dev/null; rm -f "$TARGET/.witsos/daemon.sock" 2>/dev/null; sleep 1
