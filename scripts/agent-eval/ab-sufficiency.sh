...
# witsos (explore/node), does it still Read? Premise under test: explore/node
# return source WITH line numbers, so a Read should not be needed.
#
# WITH witsos (pre-warmed daemon, reliable nested attach) vs WITHOUT (empty
...
# (configs, docs, a file witsos didn't index).
#
...
[ -d "$REPO/.witsos" ] || { echo "no .witsos index at $REPO"; exit 1; }
...
rm -rf "$TGT"
rsync -a --exclude node_modules --exclude .git --exclude dist --exclude .witsos "$REPO/" "$TGT/"
...
printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TGT" > "$OUT/mcp-cg.json"

prewarm(){
  pkill -9 -f "serve --mcp --path $TGT" 2>/dev/null
  WITSOS_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$TGT" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.witsos/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$TGT" >/dev/null 2>&1
}
...
echo "== WITH witsos (premise: explore/node used -> Read ~0) =="; run with "$OUT/mcp-cg.json" 1
