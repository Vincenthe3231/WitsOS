#!/usr/bin/env bash
# FRONTLOAD arm (approach 1): witsos attached (offload-disabled) + the front-load
...
printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","WITSOS_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG"
...
prewarm() {
  ...
  env WITSOS_OFFLOAD_DISABLE=1 WITSOS_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$1" </dev/null >/dev/null 2>&1 &
