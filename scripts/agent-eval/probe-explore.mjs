...
// without a full agent run. Usage: node probe-explore.mjs <repo-with-.witsos> "<query>"
...
if (typeof WitsOS?.openSync !== 'function') {
  console.error('could not resolve WitsOS.openSync; index keys:', Object.keys(idx), 'default keys:', idx.default && Object.keys(idx.default));
  process.exit(2);
}
...
const res = await h.execute('WitsOS_explore', { query });
