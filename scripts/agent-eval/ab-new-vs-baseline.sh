#!/usr/bin/env bash
# A/B a witsos retrieval/steering change: the NEW build (current HEAD) vs a
# BASELINE build (a git ref) — BOTH with witsos attached — on the same
# implementation task, measuring how many Read vs witsos calls the agent
...
# session): each arm PRE-WARMS a persistent witsos daemon for its target so
# claude connects to an already-bound, index-loaded daemon instantly — before
# the agent's first turn — and SKIPS witsos's startup re-exec via
# WITSOS_WASM_RELAUNCHED=1. Without this, on a multi-step task the agent
# dives into Read/grep before witsos finishes its ~2-3s startup (worse under
# the CPU contention of a nested run) and runs with NO witsos.
#
# Gotcha: claude's `system/init` snapshot can read status:"pending" / 0 tools
# even when the server then connects fine — judge by ACTUAL witsos usage in
# parse-run.mjs's "by type", not the init line.
#
# Usage: ab-new-vs-baseline.sh <indexed-repo> "<task>" [baseline-ref]
#   <indexed-repo>  a repo with a .witsos index (copied per arm)
...
[ -d "$TARGET/.witsos" ] || { echo "target not indexed: run 'witsos init $TARGET' first"; exit 1; }
...
rsync -a --exclude node_modules --exclude .git --exclude dist --exclude .witsos "$TARGET/" "$OUT/t-new/"
...
prewarm() { # target — spawn a persistent daemon (current $BIN) and wait for its socket
  ...
  WITSOS_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$1" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.witsos/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$1" \
    && echo "  daemon warm: $1" || echo "  WARN: daemon never bound for $1 (arm may run without witsos)"
}
...
run_arm() { # label, target-copy
  ...
  printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$tgt" > "$c"
  prewarm "$tgt"
  echo "############## ARM [$label] ##############"
...
echo "###### (especially Read vs mcp__witsos__*). Full logs in: $OUT"
