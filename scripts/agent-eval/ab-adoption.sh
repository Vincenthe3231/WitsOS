#!/usr/bin/env bash
# Does the agent PICK witsos_node to read a file, vs the built-in Read tool?
# Build A/B: NEW build (HEAD, witsos_node has Read parity) vs BASELINE build
# (a ref where it doesn't), BOTH witsos-attached + pre-warmed, same task. The
# metric is tool CHOICE: Read calls vs witsos_node[file] calls per run.
#
...
prewarm() {
  ...
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.witsos/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$1" >/dev/null 2>&1
}

# Per-run tool-choice counts: Read vs witsos_node[file] vs [symbol].
count() {
  node -e '
    const fs=require("fs");
    ...
    console.log(`    Read=${read}  witsos_node[file]=${cgFile}  witsos_node[symbol]=${cgSym}  other_cg=${cgOther}  (cg exposed=${exposed})`);
  ' "$1"
}
...
run_arm() { # label, N
  ...
  for i in $(seq 1 "$n"); do
    ...
    rm -rf "$tgt"
    rsync -a --exclude node_modules --exclude .git --exclude dist --exclude .witsos "$TARGET/" "$tgt/"
...
echo "== NEW build (HEAD: witsos_node has Read parity) =="
...
echo "###### DONE — compare [new] vs [baseline]: does witsos_node[file] rise / Read fall? Logs: $OUT"
