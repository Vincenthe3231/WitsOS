...
//   1. Trace adoption — on a flow question, does the with-arm actually call witsos_trace?
...
const MD = new URL('../../docs/benchmarks/witsos-ab-matrix.md', import.meta.url).pathname;
...
const cgShort = (n) => n.replace('mcp__witsos__witsos_', '').replace('mcp__witsos__', '');
const tag = (n) => n === 'Read' ? 'R' : n === 'Grep' ? 'G' : n === 'Glob' ? 'Gl'
  : n === 'Bash' ? 'B' : n === 'Task' ? 'Ag' : n === 'ToolSearch' ? 'TS'
  : n.includes('witsos') ? cgShort(n) : n;
...
function parse(file) {
  ...
  for (const l of lines) {
    ...
    if (ev.type === 'system' && ev.subtype === 'init') initCg = (ev.tools || []).filter(t => /witsos/.test(t)).length;
  ...
  const cg = calls.filter(c => c.name.includes('witsos'));
  const perTool = {};
...
console.log('\n=== PER-CELL: with-arm witsos sequence + payload (sorted by repo size) ===');
...
console.log('\n=== witsos tool usage across all cells (n calls, avg payload/call) ===');
...
console.log(`total witsos calls=${wCalls} (avg ${(wCalls / cells.length).toFixed(1)}/cell)`);
