...
# witsos (explore/node) to understand before editing, does it still Read? Like
# ab-sufficiency.sh but copies+indexes a FRESH target per run (the agent mutates
# it), so runs don't see each other's edits.
#
# WITH witsos (pre-warmed) vs WITHOUT (empty MCP), N runs each. Reports
...
run(){ # label, withCodegraph(0/1)
  ...
  for i in $(seq 1 "$RUNS"); do
    ...
    if [ "$wcg" = "1" ]; then
      printf '{"mcpServers":{"witsos":{"command":"env","args":["WITSOS_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$tgt" > "$cfg"
      prewarm "$tgt"
