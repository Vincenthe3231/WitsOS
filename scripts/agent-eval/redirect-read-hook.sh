...
# onto witsos_node during implementation, not just for Q&A).
#
# When the agent Reads a SOURCE file, deny it and steer to witsos_node's
...
# A redirect to a file witsos hasn't indexed SELF-CORRECTS: the file-view
...
# Eval artifact only. The production version is an indexed-aware `witsos`
