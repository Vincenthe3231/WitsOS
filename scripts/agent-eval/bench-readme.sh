#!/usr/bin/env bash
# Re-run the README "Benchmark Results" A/B (with vs without witsos) on the
...
# $CORPUS (default /tmp/witsos-corpus) by the build under test.
set -uo pipefail
H="$(cd "$(dirname "$0")" && pwd)"
C="${CORPUS:-/tmp/witsos-corpus}"
RUNS="${RUNS:-4}"
ROWS=(
