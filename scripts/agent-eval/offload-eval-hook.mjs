...
// Front-loads witsos's structural answer for flow/impact/"how/where" prompts so the
...
for (let i = 0; i < 6 && root; i++) {
  if (existsSync(join(root, '.witsos'))) { found = root; break; }
  const parent = resolve(root, '..'); if (parent === root) break; root = parent;
}
if (!found) { dbg(`no .witsos found from cwd=${cwd}`); process.exit(0); }
dbg(`found index at ${found}`);

try {
  process.env.WITSOS_OFFLOAD_DISABLE = '1'; // raw, accurate — never the unfixed offload
  process.env.WITSOS_TELEMETRY = '0'; process.env.DO_NOT_TRACK = '1';
  ...
  if (typeof WitsOS?.openSync !== 'function' || typeof ToolHandler !== 'function') process.exit(0);
  ...
  process.stdout.write(
    `## WitsOS structural context (auto-retrieved for this question)\n` +
    `The code graph was queried for your question; the relevant symbols, source, and call flow are below. ` +
    `Treat the quoted source as already read. If you need more, call witsos_explore with specific symbol names rather than grepping or reading files.\n\n` +
    text + '\n'
  );
