#!/usr/bin/env bash
# Build the current branch and link it as the global `witsos` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global witsos -> $(command -v witsos)"
  exit 0
fi
...
LINKED=$(command -v witsos || echo "(not on PATH)")
echo
echo "✓ global witsos now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"
