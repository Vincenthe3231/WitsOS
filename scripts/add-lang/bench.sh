...
#   clone -> wipe+index (with the witsos on PATH) -> verify extraction ->
#   with/without retrieval A/B (reuses scripts/agent-eval/run-all.sh).
#
# Assumes the witsos dev build is already built + linked on PATH — the skill
...
# Env:   CORPUS   corpus dir (default /tmp/witsos-corpus, shared with agent-eval)
set -uo pipefail
...
CORPUS="${CORPUS:-/tmp/witsos-corpus}"
REPO="$CORPUS/$NAME"

command -v witsos >/dev/null || { echo "no witsos on PATH (build + ./scripts/local-install.sh first)"; exit 1; }

echo "==================== add-lang bench: $NAME ($LANG_TOKEN) ===================="
echo "witsos: $(command -v witsos) -> $(witsos --version 2>/dev/null || echo '?')"
...
echo "→ wiping .witsos and indexing"
rm -rf "$REPO/.witsos"
( cd "$REPO" && witsos init -i ) || { echo "indexing failed"; exit 1; }
