#!/usr/bin/env node
// Sanity-check that witsos extracted REAL symbols (not just file/import nodes)
...
// Reads `witsos status <repo> --json` using whatever witsos is on PATH,
...
let status;
try {
  const out = execFileSync('witsos', ['status', repo, '--json'], { encoding: 'utf8' });
  status = JSON.parse(out);
} catch (e) {
  console.error(`[verify] could not read witsos status for ${repo}: ${e.message}`);
  process.exit(2);
}

// Kinds that prove the extractor mapped AST node types (everything except
// 'file' and 'import', which witsos creates structurally for any language).
