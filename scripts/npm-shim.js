...
'use strict';
//
// npm thin-installer launcher for WitsOS.
//
// The heavy artifact (a vendored Node runtime + the app) ships as a per-platform
// optionalDependency: @colbymchenry/witsos-<platform>-<arch>. npm installs
...
//   WITSOS_NO_DOWNLOAD=1     disable the network fallback (print guidance)
//   WITSOS_INSTALL_DIR=DIR   cache location (default: ~/.witsos)
//   WITSOS_DOWNLOAD_BASE=URL release-download base (for mirrors/air-gapped)
//
// Wired up at release time as the main package's `bin`:
//   "bin": { "witsos": "npm-shim.js" }
...
var pkg = '@colbymchenry/witsos-' + target;
var isWindows = process.platform === 'win32';
var REPO = 'colbymchenry/witsos';

main().catch(function (e) {
  process.stderr.write('witsos: ' + (e && e.message ? e.message : String(e)) + '\n');
  process.exit(1);
});
...
async function main() {
  ...
  if (res.error) {
    process.stderr.write('witsos: ' + res.error.message + '\n');
    process.exit(1);
  }
...
function resolveInstalledBundle() {
  try {
    if (isWindows) {
      ...
      var entry = require.resolve(pkg + '/lib/dist/bin/witsos.js');
      return { command: nodeExe, args: liftoff(entry) };
    }
    return { command: require.resolve(pkg + '/bin/witsos'), args: process.argv.slice(2) };
  } catch (e) {
    return null;
  }
}
...
function launcherIn(dir) {
  if (isWindows) {
    ...
    var entry = path.join(dir, 'lib', 'dist', 'bin', 'witsos.js');
    if (fs.existsSync(nodeExe) && fs.existsSync(entry)) {
      return { command: nodeExe, args: liftoff(entry) };
    }
  } else {
    var launcher = path.join(dir, 'bin', 'witsos');
...
// avoid the Zone OOM on Node >= 22 (issues #293/#298). The unix bin/witsos
...
async function selfHealBundle() {
  ...
  var bundlesDir = path.join(process.env.WITSOS_INSTALL_DIR || path.join(os.homedir(), '.witsos'), 'bundles');
  var dest = path.join(bundlesDir, target + '-' + version);

  // Already downloaded by a previous run? Use it even when downloads are
  // disabled — WITSOS_NO_DOWNLOAD blocks fetching, not a cached bundle.
  var cached = launcherIn(dest);
  if (cached) return cached;

  if (process.env.WITSOS_NO_DOWNLOAD) {
    fail('the network fallback is disabled (WITSOS_NO_DOWNLOAD is set).');
  }

  var asset = 'witsos-' + target + (isWindows ? '.zip' : '.tar.gz');
  var base = process.env.WITSOS_DOWNLOAD_BASE || ('https://github.com/' + REPO + '/releases/download');
  var url = base + '/v' + version + '/' + asset;

  process.stderr.write(
    'witsos: platform bundle missing (registry did not provide ' + pkg + ').\n' +
    'witsos: downloading ' + asset + ' from GitHub Releases (' + version + ')...\n'
  );

  // Stage inside bundlesDir so the final rename is on the same filesystem (atomic,
  // no EXDEV across tmpfs). Strip the archive's top-level witsos-<target>/ dir.
  ...
  process.stderr.write('witsos: bundle ready.\n');
  return ready;
}
...
function download(url, dest, redirectsLeft) {
  return new Promise(function (resolve, reject) {
    ...
    var req = https.get(url, { headers: { 'User-Agent': 'witsos-npm-shim' }, timeout: 30000 }, function (res) {
...
async function verifyChecksum(archivePath, asset, base, version) {
  ...
  process.stderr.write('witsos: checksum verified.\n');
}
...
function fail(reason) {
  process.stderr.write(
    'witsos: no prebuilt bundle for ' + target + '.\n' +
    (reason ? 'witsos: ' + reason + '\n' : '') +
    ...
    '      npm i -g @colbymchenry/witsos --registry=https://registry.npmjs.org\n' +
