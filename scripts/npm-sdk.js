'use strict';
//
// Programmatic / embedded SDK entry for @colbymchenry/witsos (issue #354).
//
...
function resolveLibrary() {
  ...
  throw new Error(
    'witsos: the programmatic API is unavailable because the platform bundle\n' +
    ...
    '      npm i @colbymchenry/witsos --registry=https://registry.npmjs.org\n' +
    '  - or run the CLI once (e.g. `npx @colbymchenry/witsos status`) to\n' +
    '    self-heal the bundle into ~/.witsos, then require() will find it.'
  );
}
...
function cachedLibrary() {
  try {
    ...
    var base = process.env.WITSOS_INSTALL_DIR || path.join(os.homedir(), '.witsos');
